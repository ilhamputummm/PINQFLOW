...
function More(props){
  ...
  return(
    <div style={{padding:16}}>
      ...
      {/* User info + logout */}
      <div style={{...card,padding:"16px 18px",marginBottom:16,display:"flex",alignItems:"center",gap:14}}>
        ...
        <div style={{flex:1}}>
          ...
          <div style={{fontSize:11,color:C.muted,marginTop:1}}>Data tersimpan terpisah 🔒</div>
        </div>
        <button onClick={onLogout} style={{background:`${C.coral}12`,border:`1px solid ${C.coral}30`,borderRadius:12,padding:"8px 14px",color:C.coral,fontSize:12,fontWeight:700,cursor:"pointer",flexShrink:0}}>
          Ganti Akun
        </button>
      </div>
...
const DEFAULT_ACCOUNTS = [
  {id:"u1",name:"User 1",emoji:"🐧",color:"#10B981"},
  {id:"u2",name:"User 2",emoji:"🦊",color:"#F97316"},
  {id:"u3",name:"User 3",emoji:"🐻",color:"#8B5CF6"},
  {id:"u4",name:"User 4",emoji:"🦋",color:"#EC4899"},
  {id:"u5",name:"User 5",emoji:"🐬",color:"#0EA5E9"},
];
...
function LoginScreen({onLogin,C}){
  ...
  const startEdit=(e,acc)=>{e.stopPropagation();setEditing(acc.id);setEditName(acc.name);setEditEmoji(acc.emoji);};
  const saveEdit=async()=>{
    const updated=accounts.map(a=>a.id===editing?{...a,name:editName||a.name,emoji:editEmoji||a.emoji}:a);
    setAccounts(updated);await saveAccounts(updated);setEditing(null);
  };
  if(!accounts)return<div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",background:C.bg}}><div style={{fontSize:13,color:C.muted}}>Loading...</div></div>;
  return(
    <div style={{position:"fixed",inset:0,background:C.bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"'DM Sans',system-ui,sans-serif",padding:"0 24px"}}>
      <div style={{width:"100%",maxWidth:400}}>
        <div style={{textAlign:"center",marginBottom:36}}>
          <svg width={64} height={64} viewBox="0 0 100 110" xmlns="http://www.w3.org/2000/svg" style={{margin:"0 auto 14px",display:"block",filter:`drop-shadow(0 8px 20px ${C.mint}50)`}}>
          ...
          <div style={{fontSize:26,fontWeight:900,letterSpacing:-1,color:C.navy,lineHeight:1.1}}><span style={{color:"#F87171"}}>Pinq</span><span style={{color:"#10B981"}}>Flow</span></div>
          <div style={{fontSize:12,color:C.muted,marginTop:6,fontWeight:500}}>Pilih akun kamu untuk mulai 🐧</div>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {accounts.map(acc=>(
            <div key={acc.id}>
              {editing===acc.id?(
                <div style={{background:C.bgCard,borderRadius:20,padding:"16px 18px",boxShadow:C.shadow,border:`2px solid ${acc.color}`}}>
                  <div style={{fontSize:12,fontWeight:700,color:C.muted,letterSpacing:1,marginBottom:10}}>EDIT PROFIL</div>
                  <div style={{display:"flex",gap:8,marginBottom:10,flexWrap:"wrap"}}>
                    {EMOJIS.map(e=>(
                      <button key={e} onClick={()=>setEditEmoji(e)} style={{width:36,height:36,borderRadius:10,border:`2px solid ${editEmoji===e?acc.color:"transparent"}`,background:editEmoji===e?`${acc.color}20`:"transparent",fontSize:18,cursor:"pointer"}}>{e}</button>
                    ))}
                  </div>
                  <input value={editName} onChange={e=>setEditName(e.target.value)} placeholder="Nama akun..." maxLength={20}
                    style={{width:"100%",background:C.inpBg,border:`1.5px solid ${C.border}`,borderRadius:12,padding:"11px 14px",color:C.navy,fontSize:14,fontWeight:600,boxSizing:"border-box",outline:"none",fontFamily:"inherit",marginBottom:10}}/>
                  <div style={{display:"flex",gap:8}}>
                    <button onClick={()=>setEditing(null)} style={{flex:1,background:C.border,border:"none",borderRadius:12,padding:"10px 0",color:C.muted,fontSize:13,fontWeight:700,cursor:"pointer"}}>Batal</button>
                    <button onClick={saveEdit} style={{flex:2,background:`linear-gradient(135deg,${acc.color},${acc.color}cc)`,border:"none",borderRadius:12,padding:"10px 0",color:"#fff",fontSize:13,fontWeight:800,cursor:"pointer"}}>Simpan</button>
                  </div>
                </div>
              ):(
                <button onClick={()=>onLogin(acc)} style={{width:"100%",background:C.bgCard,border:`1.5px solid ${C.border}`,borderRadius:20,padding:"16px 18px",cursor:"pointer",display:"flex",alignItems:"center",gap:14,boxShadow:C.shadow,textAlign:"left",transition:"all .18s",position:"relative"}}>
                  <div style={{width:52,height:52,borderRadius:16,background:`${acc.color}18`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:26,flexShrink:0}}>{acc.emoji}</div>
                  <div style={{flex:1}}>
                    <div style={{fontSize:16,fontWeight:800,color:C.navy,letterSpacing:-0.3}}>{acc.name}</div>
                    <div style={{fontSize:11,color:C.muted,marginTop:2,fontWeight:500}}>Ketuk untuk masuk →</div>
                  </div>
                  <button onClick={e=>startEdit(e,acc)} style={{background:"none",border:`1px solid ${C.border}`,borderRadius:10,width:32,height:32,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0,color:C.muted,fontSize:14}}>✏️</button>
                </button>
              )}
            </div>
          ))}
        </div>
        <div style={{textAlign:"center",marginTop:24,fontSize:11,color:C.muted,lineHeight:1.6}}>
          Data setiap akun tersimpan terpisah 🔒<br/>Bisa diakses dari HP manapun
        </div>
      </div>
    </div>
  );
