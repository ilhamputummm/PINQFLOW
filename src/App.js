...
function Transactions({txs,data,updateData}){
  ...
  return(
    <div style={{padding:"16px 14px 100px"}}>
      ...
      {list.map((t)=>(
        <div key={t.id} data-txcard="1" style={{...cardSm,padding:"13px 14px",marginBottom:8,display:"flex",gap:12,alignItems:"center"}}>
...
function UserApp({user,onLogout}){
  ...
  useEffect(()=>{
    loadData(user.id).then(d=>{
      ...
      // Auto-embed Putu's historical data if not yet imported
      const isPutu=user?.name?.toUpperCase()==="PUTU"||user?.id==="u1";
      if(isPutu){
        const existingKeys=new Set(applied.transactions.map(t=>t.date+"_"+t.description+"_"+t.amount));
        const fresh=PUTU_IMPORT_DATA.filter(t=>!existingKeys.has(t.date+"_"+t.description+"_"+t.amount));
        if(fresh.length>0){
          const merged={...applied,transactions:[...applied.transactions,...fresh].sort((a,b)=>a.date.localeCompare(b.date))};
