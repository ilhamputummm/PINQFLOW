...
// ─── IMPORT PAGE ──────────────────────────────────────────────────────────────
function ImportPage({data,updateData,onBack}){
  const C=useTheme();
  const card=mkCard(C),cardSm=mkCardSm(C);
  const [status,setStatus]=useState("idle"); // idle | preview | importing | done | undone
  const [previewTxs,setPreviewTxs]=useState([]);
  const [importedIds,setImportedIds]=useState([]);

  const handlePreview=()=>{
    // Filter out already imported transactions
    const existingDescs=new Set(data.transactions.map(t=>t.date+"_"+t.description+"_"+t.amount));
    const fresh=PUTU_IMPORT_DATA.filter(t=>!existingDescs.has(t.date+"_"+t.description+"_"+t.amount));
    setPreviewTxs(fresh);
    setStatus("preview");
  };
  ...
  const handleImport=async()=>{
    setStatus("importing");
    const ids=previewTxs.map(t=>t.id);
    setImportedIds(ids);
    const merged={...data,transactions:[...data.transactions,...previewTxs].sort((a,b)=>a.date.localeCompare(b.date))};
    await updateData(merged);
    setStatus("done");
  };

  const handleUndo=async()=>{
    const restored={...data,transactions:data.transactions.filter(t=>!importedIds.includes(t.id))};
  ...
  const alreadyImported=data.transactions.some(t=>t.notes&&t.notes.includes("Imported from"));

  return(
    <div style={{padding:"0 0 80px"}}>
      <BackBtn onBack={onBack}/>
      <div style={{fontSize:18,fontWeight:900,color:C.navy,marginBottom:6,letterSpacing:-0.5}}>📥 Import Data</div>
      <div style={{fontSize:12,color:C.muted,marginBottom:20}}>Import your Jan–May 2026 spreadsheet data</div>

      {status==="idle"&&(
        <>
          {alreadyImported&&(
            <div style={{...cardSm,padding:"12px 16px",marginBottom:14,background:`${C.gold}15`,border:`1px solid ${C.gold}40`}}>
              <div style={{fontSize:12,fontWeight:700,color:C.gold,marginBottom:3}}>⚠️ Already Imported</div>
              <div style={{fontSize:11,color:C.muted}}>Some data has already been imported. Duplicate entries will be skipped automatically.</div>
            </div>
          )}
          <div style={{...card,padding:"18px 20px",marginBottom:14}}>
            <div style={{fontSize:13,fontWeight:700,color:C.navy,marginBottom:12}}>📊 Data Summary</div>
            {[
              {m:"January 2026",count:92},
              {m:"February 2026",count:75},
              {m:"March 2026",count:76},
              {m:"April 2026",count:67},
              {m:"May 2026",count:31},
            ].map(({m,count})=>(
              <div key={m} style={{display:"flex",justifyContent:"space-between",padding:"7px 0",borderBottom:`1px solid ${C.border}`}}>
                <span style={{fontSize:13,color:C.navyMid,fontWeight:600}}>{m}</span>
                <span style={{fontSize:12,color:C.muted}}>{count} transactions</span>
              </div>
            ))}
            <div style={{display:"flex",justifyContent:"space-between",padding:"10px 0 0"}}>
              <span style={{fontSize:13,fontWeight:800,color:C.navy}}>Total</span>
              <span style={{fontSize:13,fontWeight:800,color:C.mintD}}>342 transactions</span>
            </div>
          </div>
          <div style={{...cardSm,padding:"12px 16px",marginBottom:20,background:`${C.blue}10`,border:`1px solid ${C.blue}30`}}>
            <div style={{fontSize:11,color:C.blue,fontWeight:700,marginBottom:4}}>ℹ️ Category Mapping</div>
            <div style={{fontSize:11,color:C.muted,lineHeight:1.7}}>
              Lifestyle → Dining & Food<br/>
              Personal Need's Food → Groceries<br/>
              Transport/Gas/Tol → Transport & Fuel<br/>
              Pulsa & Internet → Mobile Data & Internet<br/>
              Monthly Saving's → Savings
            </div>
          </div>
          <button onClick={handlePreview} style={{width:"100%",background:`linear-gradient(135deg,${C.mint},${C.mintD})`,border:"none",borderRadius:16,padding:"16px 0",color:"#fff",fontSize:15,fontWeight:800,cursor:"pointer",boxShadow:`0 6px 20px ${C.mint}40`}}>
            Preview Import →
          </button>
        </>
      )}
      ...
      {status==="preview"&&(
        <>
          <div style={{...card,padding:"16px 18px",marginBottom:14}}>
            <div style={{fontSize:13,fontWeight:700,color:C.navy,marginBottom:12}}>Preview by Month</div>
            {Object.entries(byMonth).sort().map(([m,d])=>{
              const [y,mo]=m.split("-");
              const months=["","January","February","March","April","May","June","July","August","September","October","November","December"];
              return(
                <div key={m} style={{marginBottom:12}}>
                  <div style={{fontSize:12,fontWeight:700,color:C.navy,marginBottom:6}}>{months[+mo]} {y} <span style={{fontSize:10,color:C.muted,fontWeight:500}}>({d.count} items)</span></div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                    <div style={{background:`${C.mintD}12`,borderRadius:10,padding:"8px 10px"}}>
                      <div style={{fontSize:9,color:C.muted,fontWeight:700,letterSpacing:1,marginBottom:2}}>INCOME</div>
                      <div style={{fontFamily:"'Space Grotesk',sans-serif",fontSize:12,fontWeight:700,color:C.mintD}}>{rpK(d.inc)}</div>
                    </div>
                    <div style={{background:`${C.coral}12`,borderRadius:10,padding:"8px 10px"}}>
                      <div style={{fontSize:9,color:C.muted,fontWeight:700,letterSpacing:1,marginBottom:2}}>EXPENSE</div>
                      <div style={{fontFamily:"'Space Grotesk',sans-serif",fontSize:12,fontWeight:700,color:C.coral}}>{rpK(d.exp)}</div>
                    </div>
                  </div>
                </div>
              );
            ...
            <div style={{borderTop:`1px solid ${C.border}`,paddingTop:10,marginTop:4}}>
              <div style={{fontSize:12,fontWeight:700,color:C.navy}}>{previewTxs.length} new transactions will be imported</div>
              <div style={{fontSize:11,color:C.muted,marginTop:2}}>You can undo this anytime after import</div>
            </div>
          </div>
          <div style={{display:"flex",gap:10}}>
            <button onClick={()=>setStatus("idle")} style={{flex:1,background:C.border,border:"none",borderRadius:14,padding:"14px 0",color:C.muted,fontSize:14,fontWeight:700,cursor:"pointer"}}>Back</button>
            <button onClick={handleImport} style={{flex:2,background:`linear-gradient(135deg,${C.mint},${C.mintD})`,border:"none",borderRadius:14,padding:"14px 0",color:"#fff",fontSize:14,fontWeight:800,cursor:"pointer",boxShadow:`0 6px 20px ${C.mint}40`}}>
              Import Now ✓
            </button>
          </div>
        </>
      ...
      {status==="importing"&&(
        <div style={{textAlign:"center",padding:"40px 0"}}>
          ...
          <div style={{fontSize:14,fontWeight:700,color:C.navy,marginBottom:4}}>Importing to Firebase...</div>
      ...
      {status==="done"&&(
        <>
          <div style={{...card,padding:"20px",marginBottom:16,textAlign:"center"}}>
            <div style={{fontSize:40,marginBottom:12}}>✅</div>
            <div style={{fontSize:16,fontWeight:900,color:C.navy,marginBottom:4}}>Import Successful!</div>
            <div style={{fontSize:12,color:C.muted,marginBottom:12}}>{previewTxs.length} transactions imported from Jan–May 2026</div>
            <div style={{fontSize:11,color:C.muted,background:C.inpBg,borderRadius:10,padding:"8px 12px"}}>
              Data is now saved to your Firebase account
            </div>
          </div>
          <button onClick={handleUndo} style={{width:"100%",background:`${C.coral}12`,border:`1.5px solid ${C.coral}40`,borderRadius:16,padding:"14px 0",color:C.coral,fontSize:14,fontWeight:700,cursor:"pointer"}}>
            ↩ Undo Import
          </button>
        </>
      )}
      ...
      {status==="undone"&&(
        <div style={{...card,padding:"20px",textAlign:"center"}}>
          <div style={{fontSize:40,marginBottom:12}}>↩</div>
          <div style={{fontSize:16,fontWeight:900,color:C.navy,marginBottom:4}}>Import Undone</div>
          <div style={{fontSize:12,color:C.muted}}>All imported transactions have been removed</div>
        </div>
      )}
    </div>
  );
...
function More(props){
  ...
  const PAGES={
    ...
    import:{title:"📥 Import Data",comp:<ImportPage data={data} updateData={updateData} onBack={()=>setPage("menu")}/>},
  };
  if(page!=="menu")return(
    ...
    <div style={{padding:16}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
        {[
          ...
          ...(user?.name?.toUpperCase()==="PUTU"||user?.id==="u1"?[{id:"import",e:"📥",t:"Import Data",d:"Jan–May 2026"}]:[]),
        ].map(item=>(
...
const PUTU_IMPORT_DATA = [{"id":"imp616442071","date":"2025-03-26","description":"Biwir","category":"Ritual Penggandaan Rezeki","amount":150000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp637591863","date":"2026-01-01","description":"S 18 December","category":"Other Income","amount":1000000,"type":"income","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp576071863","date":"2026-01-01","description":"Tanmu Coffee","category":"Dining & Food","amount":78000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp321671863","date":"2026-01-01","description":"S 19 December","category":"Other Income","amount":400000,"type":"income","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp749731863","date":"2026-01-01","description":"Baso Holly","category":"Groceries","amount":30000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp750341863","date":"2026-01-01","description":"SPPD 19 December","category":"Other Income","amount":1677000,"type":"income","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp724431863","date":"2026-01-01","description":"Terea","category":"Dining & Food","amount":70000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp763081863","date":"2026-01-01","description":"Akhir Taun O","category":"Other Income","amount":300000,"type":"income","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp563551863","date":"2026-01-01","description":"Syifa","category":"Ritual Penggandaan Rezeki","amount":3000000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp601391863","date":"2026-01-01","description":"Monyh Salary","category":"Other Income","amount":4800000,"type":"income","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp991281863","date":"2026-01-01","description":"M","category":"Other Income","amount":500000,"type":"income","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp612851863","date":"2026-01-02","description":"E Tol","category":"Transport & Fuel","amount":300000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp853201863","date":"2026-01-02","description":"Coffee Companion","category":"Dining & Food","amount":95000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp515511863","date":"2026-01-02","description":"Bensin Mobil","category":"Transport & Fuel","amount":280000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp870051863","date":"2026-01-03","description":"Token Listrik Sentul","category":"Mobile Data & Internet","amount":306700,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp922771863","date":"2026-01-03","description":"Bensin Motor","category":"Transport & Fuel","amount":90000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp280121863","date":"2026-01-03","description":"Duren Cimahpar","category":"Groceries","amount":130000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp520371863","date":"2026-01-04","description":"Uang Kebersamaan Star House","category":"Dining & Food","amount":110000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp402841863","date":"2026-01-04","description":"Indomaret","category":"Dining & Food","amount":103000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp664211863","date":"2026-01-05","description":"M","category":"Other Income","amount":500000,"type":"income","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp922101863","date":"2026-01-05","description":"M","category":"Other Income","amount":268000,"type":"income","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp387881863","date":"2026-01-06","description":"Terea","category":"Dining & Food","amount":70000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp476981863","date":"2026-01-06","description":"Apple Subscribe (DANA)","category":"Subscriptions","amount":200000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp259071863","date":"2026-01-06","description":"Tanaman Alocasia","category":"Hobbies","amount":19000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp628511863","date":"2026-01-07","description":"Kopi Warung","category":"Dining & Food","amount":12000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp824921863","date":"2026-01-07","description":"Makan Warung","category":"Groceries","amount":15000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp841501863","date":"2026-01-07","description":"Pot Tanaman","category":"Hobbies","amount":21195,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp372611863","date":"2026-01-08","description":"Case iPhone Spigen","category":"Shopping","amount":315944,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp508331863","date":"2026-01-08","description":"Media Tanam","category":"Hobbies","amount":27800,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp501031863","date":"2026-01-08","description":"Sekop","category":"Hobbies","amount":13060,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp112361863","date":"2026-01-08","description":"Monstera Monthai","category":"Hobbies","amount":94400,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp387021863","date":"2026-01-08","description":"WiFi Indihome","category":"Mobile Data & Internet","amount":143824,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp793771863","date":"2026-01-09","description":"Makan Kopi Warung","category":"Groceries","amount":21000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp681981863","date":"2026-01-09","description":"Semprotan Tanaman","category":"Hobbies","amount":55800,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp330651863","date":"2026-01-09","description":"Kepala Charger UGreen","category":"Shopping","amount":166469,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp595461863","date":"2026-01-09","description":"Shell","category":"Transport & Fuel","amount":329819,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp763211863","date":"2026-01-10","description":"Terea","category":"Dining & Food","amount":70000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp652671863","date":"2026-01-11","description":"Microsoft","category":"Shopping","amount":130000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp582121863","date":"2026-01-11","description":"Gopay TopUp (Newport)","category":"Subscriptions","amount":116000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp876131863","date":"2026-01-11","description":"P2P Aquarium","category":"Hobbies","amount":20000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp335931863","date":"2026-01-11","description":"E Tol","category":"Transport & Fuel","amount":300000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp216961863","date":"2026-01-12","description":"M","category":"Other Income","amount":500000,"type":"income","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp865321863","date":"2026-01-12","description":"Kopi Warung","category":"Dining & Food","amount":4000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp569581863","date":"2026-01-12","description":"Taneman Veitchii & Alocasia","category":"Hobbies","amount":130234,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp426091863","date":"2026-01-12","description":"Makan Warung","category":"Groceries","amount":15000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp289141863","date":"2026-01-12","description":"Terea","category":"Dining & Food","amount":35000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp418311863","date":"2026-01-13","description":"Kopi Warung","category":"Dining & Food","amount":4000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp850201863","date":"2026-01-14","description":"Terea","category":"Dining & Food","amount":70000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp502491863","date":"2026-01-14","description":"Kopi Warung","category":"Dining & Food","amount":4000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp536831863","date":"2026-01-15","description":"S","category":"Other Income","amount":600000,"type":"income","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp775951863","date":"2026-01-15","description":"Kopi Warung","category":"Dining & Food","amount":8000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp453161863","date":"2026-01-15","description":"Bensin","category":"Transport & Fuel","amount":350000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp586991863","date":"2026-01-15","description":"E Tol","category":"Transport & Fuel","amount":198500,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp809531863","date":"2026-01-16","description":"Tambal Ban","category":"Transport & Fuel","amount":25000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp118681863","date":"2026-01-16","description":"Dimitree","category":"Dining & Food","amount":169400,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp465681863","date":"2026-01-16","description":"Kouta Telkomsel","category":"Mobile Data & Internet","amount":90000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp459131863","date":"2026-01-17","description":"F","category":"Other Income","amount":330000,"type":"income","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp649191863","date":"2026-01-17","description":"Pasang Meja","category":"Ritual Penggandaan Rezeki","amount":100000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp148891863","date":"2026-01-17","description":"Irman","category":"Ritual Penggandaan Rezeki","amount":100000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp176091863","date":"2026-01-17","description":"Cukur","category":"Self Care","amount":40000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp268991863","date":"2026-01-17","description":"Alfamart","category":"Dining & Food","amount":108300,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp952921863","date":"2026-01-18","description":"Makan Pasar Bersih + Tuku","category":"Groceries","amount":114000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp175561863","date":"2026-01-18","description":"USB Type C Extension Cable","category":"Shopping","amount":127752,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp798441863","date":"2026-01-20","description":"M","category":"Other Income","amount":500000,"type":"income","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp447351864","date":"2026-01-20","description":"Kopi Roko Warung","category":"Dining & Food","amount":37000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp225931864","date":"2026-01-20","description":"Terea + Sabun","category":"Dining & Food","amount":75000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp738501863","date":"2026-01-21","description":"S","category":"Other Income","amount":200000,"type":"income","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp756421864","date":"2026-01-21","description":"Kopi Warung","category":"Dining & Food","amount":4000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp302941864","date":"2026-01-21","description":"Kunci L","category":"Shopping","amount":63350,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp769841864","date":"2026-01-23","description":"Terea","category":"Dining & Food","amount":90000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp455601864","date":"2026-01-23","description":"Game Uncharted","category":"Hobbies","amount":25900,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp208001864","date":"2026-01-23","description":"Tissue+Sayur","category":"Shopping","amount":35000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp779311864","date":"2026-01-24","description":"Kerupuk","category":"Groceries","amount":10000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp124581864","date":"2026-01-24","description":"Bensin BP","category":"Transport & Fuel","amount":320000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp490481864","date":"2026-01-24","description":"Kouta XL","category":"Mobile Data & Internet","amount":80000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp168811864","date":"2026-01-25","description":"E Tol","category":"Transport & Fuel","amount":300000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp369521864","date":"2026-01-26","description":"Daging + Gas","category":"Groceries","amount":285000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp496331864","date":"2026-01-26","description":"Roko + Kopi Warung","category":"Dining & Food","amount":30000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp255771863","date":"2026-01-27","description":"M","category":"Other Income","amount":500000,"type":"income","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp610151864","date":"2026-01-27","description":"Makan + Kopi Warung","category":"Dining & Food","amount":25000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp374931864","date":"2026-01-27","description":"Terea Alfamart","category":"Dining & Food","amount":124000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp996391864","date":"2026-01-28","description":"Kopi Warung","category":"Dining & Food","amount":4000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp536941864","date":"2026-01-28","description":"FlashDisk 16Gb","category":"Shopping","amount":73400,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp251931864","date":"2026-01-28","description":"Topup Dana","category":"Subscriptions","amount":100000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp510981864","date":"2026-01-29","description":"Daging","category":"Groceries","amount":107600,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp840551864","date":"2026-01-29","description":"Testpen","category":"Shopping","amount":26900,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp970901864","date":"2026-01-30","description":"Kopi Roko  Warung","category":"Dining & Food","amount":37000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp286511864","date":"2026-01-30","description":"Terea","category":"Dining & Food","amount":71000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp123801864","date":"2026-01-30","description":"E Tol","category":"Transport & Fuel","amount":300000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp982641864","date":"2026-01-31","description":"Bensin","category":"Transport & Fuel","amount":310000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp295091864","date":"2026-01-31","description":"Listrik","category":"Mobile Data & Internet","amount":202600,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp158201864","date":"2026-01-31","description":"Bensin Vivo","category":"Transport & Fuel","amount":310000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp866551864","date":"2026-01-31","description":"Gopay TopUp (Kebab)","category":"Dining & Food","amount":81000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp805591963","date":"2026-02-01","description":"S & Arif (29&30 Januari)","category":"Other Income","amount":2300000,"type":"income","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp884591963","date":"2026-02-01","description":"Token Listrik","category":"Mobile Data & Internet","amount":203600,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp836101963","date":"2026-02-01","description":"Breza Coffee","category":"Dining & Food","amount":59000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp528981963","date":"2026-02-01","description":"Terea","category":"Dining & Food","amount":71000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp755701963","date":"2026-02-02","description":"M","category":"Other Income","amount":500000,"type":"income","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp244821963","date":"2026-02-02","description":"Month Salary","category":"Monthly Salary","amount":4500000,"type":"income","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp564271963","date":"2026-02-02","description":"Sirloin HJK","category":"Groceries","amount":87000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp374811963","date":"2026-02-02","description":"Kopi Warung","category":"Dining & Food","amount":4000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp544821963","date":"2026-02-03","description":"E Tol","category":"Transport & Fuel","amount":300000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp971241963","date":"2026-02-03","description":"Makan Warung","category":"Groceries","amount":17000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp437481963","date":"2026-02-04","description":"Kopi Warung","category":"Dining & Food","amount":4000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp413881963","date":"2026-02-04","description":"Syifa","category":"Ritual Penggandaan Rezeki","amount":3000000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp886831963","date":"2026-02-05","description":"S (3 & 5 Februari)","category":"Other Income","amount":1500000,"type":"income","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp825641963","date":"2026-02-05","description":"Kopi Warung","category":"Dining & Food","amount":4000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp739821963","date":"2026-02-05","description":"Desk Shelf","category":"Shopping","amount":113200,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp479051963","date":"2026-02-05","description":"Terea Gojek","category":"Dining & Food","amount":100000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp897581963","date":"2026-02-05","description":"Media Tanam","category":"Hobbies","amount":25000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp339961963","date":"2026-02-06","description":"Kopi Warung","category":"Dining & Food","amount":4000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp964441963","date":"2026-02-06","description":"Kokuo","category":"Self Care","amount":250000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp188891963","date":"2026-02-06","description":"Nasi Goreng","category":"Groceries","amount":30000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp375241963","date":"2026-02-06","description":"Bensin Cibubur","category":"Transport & Fuel","amount":331238,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp381641963","date":"2026-02-07","description":"Anthurium Warocq","category":"Hobbies","amount":350000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp410471963","date":"2026-02-07","description":"Topup Lalamove","category":"Hobbies","amount":50000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp257191963","date":"2026-02-08","description":"Pentol","category":"Groceries","amount":100000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp993791963","date":"2026-02-08","description":"Mie Ayam Umi","category":"Groceries","amount":36000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp168551963","date":"2026-02-08","description":"Alfamart","category":"Dining & Food","amount":110000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp700881963","date":"2026-02-08","description":"Baju Ayah","category":"Ritual Penggandaan Rezeki","amount":700000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp910501963","date":"2026-02-08","description":"WiFi Indihome","category":"Mobile Data & Internet","amount":366637,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp412031963","date":"2026-02-09","description":"E Tol","category":"Transport & Fuel","amount":360000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp289141963","date":"2026-02-09","description":"Tanaman","category":"Hobbies","amount":96075,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp204301963","date":"2026-02-09","description":"Remot AC","category":"Shopping","amount":30700,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp398821963","date":"2026-02-09","description":"Arisan","category":"Savings","amount":200000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp435791963","date":"2026-02-10","description":"M","category":"Other Income","amount":500000,"type":"income","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp116891963","date":"2026-02-10","description":"Kopi Warung","category":"Dining & Food","amount":4000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp342511963","date":"2026-02-10","description":"Token Listrik","category":"Mobile Data & Internet","amount":201100,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp177051963","date":"2026-02-11","description":"S (10&11 Februari)","category":"Other Income","amount":1200000,"type":"income","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp111011963","date":"2026-02-11","description":"Turus Lumut","category":"Hobbies","amount":34400,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp437171963","date":"2026-02-12","description":"Kopi Warung","category":"Dining & Food","amount":12000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp838191963","date":"2026-02-12","description":"FC 26","category":"Hobbies","amount":95900,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp812001963","date":"2026-02-12","description":"Terea Gopay Topup","category":"Dining & Food","amount":89000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp776961963","date":"2026-02-13","description":"Kopi Nako Gopay Topup","category":"Dining & Food","amount":116000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp140981963","date":"2026-02-13","description":"Philodendron Burle Marx","category":"Hobbies","amount":100000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp371401963","date":"2026-02-14","description":"Jasa Biwir","category":"Ritual Penggandaan Rezeki","amount":170666,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp382621963","date":"2026-02-14","description":"Sekam Bakar","category":"Hobbies","amount":23200,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp124491963","date":"2026-02-14","description":"Pupuk Kompos","category":"Hobbies","amount":66800,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp222241963","date":"2026-02-14","description":"Alat berkebun","category":"Hobbies","amount":55650,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp957091963","date":"2026-02-14","description":"Kaede Coffee","category":"Dining & Food","amount":60500,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp190791963","date":"2026-02-16","description":"Terea","category":"Dining & Food","amount":70000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp463411963","date":"2026-02-16","description":"Biwir","category":"Ritual Penggandaan Rezeki","amount":36000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp904711963","date":"2026-02-16","description":"Kawan Baru","category":"Dining & Food","amount":74700,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp206821963","date":"2026-02-16","description":"Bensin","category":"Transport & Fuel","amount":400000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp297171963","date":"2026-02-16","description":"E Tol","category":"Transport & Fuel","amount":360000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp208881963","date":"2026-02-18","description":"Munggahan Kantor","category":"Other Income","amount":300000,"type":"income","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp847081963","date":"2026-02-18","description":"Makan Warung","category":"Groceries","amount":20000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp590401963","date":"2026-02-18","description":"Shampoo Kelaya","category":"Self Care","amount":73350,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp626721963","date":"2026-02-18","description":"Terea + Cheat PB","category":"Dining & Food","amount":121000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp577391963","date":"2026-02-19","description":"M","category":"Other Income","amount":500000,"type":"income","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp903931963","date":"2026-02-20","description":"Oli & Etc. motor","category":"Transport & Fuel","amount":182195,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp171331963","date":"2026-02-20","description":"Kurma","category":"Groceries","amount":60800,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp257001963","date":"2026-02-21","description":"Gopay Topup","category":"Shopping","amount":122000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp756631963","date":"2026-02-21","description":"Pulsa XL","category":"Mobile Data & Internet","amount":100000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp487411963","date":"2026-02-21","description":"Kopi Ud Djaya","category":"Dining & Food","amount":60500,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp680211963","date":"2026-02-21","description":"Sil Tip Shockbreaker","category":"Shopping","amount":100000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp562031963","date":"2026-02-21","description":"Bensin","category":"Transport & Fuel","amount":200000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp540941963","date":"2026-02-22","description":"Belajar Clip","category":"Subscriptions","amount":103400,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp489141963","date":"2026-02-22","description":"E Tol","category":"Transport & Fuel","amount":300000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp408091963","date":"2026-02-24","description":"M","category":"Other Income","amount":500000,"type":"income","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp584391963","date":"2026-02-24","description":"Bensin","category":"Transport & Fuel","amount":347000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp792451963","date":"2026-02-24","description":"Martabak","category":"Groceries","amount":45000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp307861963","date":"2026-02-24","description":"DP Bukber 7 Maret","category":"Groceries","amount":240000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp284331963","date":"2026-02-25","description":"Shopee Pay (Ubi)","category":"Shopping","amount":31260,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp799011963","date":"2026-02-26","description":"E Tol","category":"Transport & Fuel","amount":150000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp730951963","date":"2026-02-27","description":"S","category":"Other Income","amount":100000,"type":"income","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp549401963","date":"2026-02-27","description":"Gopay Topup (Blenger)","category":"Groceries","amount":151000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp575791963","date":"2026-02-28","description":"PT PT Padel","category":"Hobbies","amount":100000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp195142071","date":"2026-03-01","description":"Token Listrik","category":"Mobile Data & Internet","amount":306700,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp283562071","date":"2026-03-02","description":"Month Salary","category":"Monthly Salary","amount":4500000,"type":"income","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp932752071","date":"2026-03-02","description":"Verdant & Uncle Jo","category":"Dining & Food","amount":105000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp956182071","date":"2026-03-02","description":"S","category":"Other Income","amount":100000,"type":"income","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp230962071","date":"2026-03-03","description":"Bensin BP","category":"Transport & Fuel","amount":330000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp405202071","date":"2026-03-03","description":"Terea & Greek Youg","category":"Shopping","amount":58900,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp543872071","date":"2026-03-04","description":"Arisan","category":"Savings","amount":200000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp441182071","date":"2026-03-05","description":"E Money","category":"Transport & Fuel","amount":300000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp895522071","date":"2026-03-05","description":"Microfiber dan Pembersih","category":"Shopping","amount":74214,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp928852071","date":"2026-03-06","description":"M","category":"Other Income","amount":350000,"type":"income","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp215822071","date":"2026-03-06","description":"Syifa","category":"Ritual Penggandaan Rezeki","amount":3000000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp347272071","date":"2026-03-06","description":"Warbew","category":"Dining & Food","amount":17000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp750282071","date":"2026-03-06","description":"Hvbitat","category":"Dining & Food","amount":235900,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp496872071","date":"2026-03-06","description":"MCD","category":"Groceries","amount":102500,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp898122071","date":"2026-03-07","description":"FC 26","category":"Shopping","amount":61900,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp199872071","date":"2026-03-07","description":"Bukber Cipete","category":"Groceries","amount":56000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp779942071","date":"2026-03-07","description":"E Tol (gantiin syifa)","category":"Transport & Fuel","amount":300000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp440572071","date":"2026-03-07","description":"Beras","category":"Groceries","amount":58000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp121282071","date":"2026-03-07","description":"Terea","category":"Dining & Food","amount":70000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp456262071","date":"2026-03-08","description":"M","category":"Other Income","amount":300000,"type":"income","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp153092071","date":"2026-03-10","description":"Apple Music (Topup Dana)","category":"Subscriptions","amount":60000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp821902071","date":"2026-03-10","description":"Konidin","category":"Self Care","amount":83229,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp893192071","date":"2026-03-11","description":"Sarung tangan cuci mobil","category":"Shopping","amount":24716,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp360782071","date":"2026-03-11","description":"Baju Syifa","category":"Shopping","amount":73654,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp773672071","date":"2026-03-11","description":"Taneman","category":"Hobbies","amount":127500,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp141662071","date":"2026-03-11","description":"Bensin","category":"Transport & Fuel","amount":342000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp948892071","date":"2026-03-12","description":"Biwir","category":"Ritual Penggandaan Rezeki","amount":72000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp805602071","date":"2026-03-13","description":"THR","category":"Other Income","amount":4400000,"type":"income","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp220162071","date":"2026-03-13","description":"Warbew","category":"Dining & Food","amount":26000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp491462071","date":"2026-03-13","description":"Terea","category":"Dining & Food","amount":70000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp788362071","date":"2026-03-13","description":"Kopi Klan","category":"Dining & Food","amount":135300,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp377832071","date":"2026-03-13","description":"Nasi Pada Trio","category":"Groceries","amount":54000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp483392071","date":"2026-03-13","description":"E Tol (gantiin syifa)","category":"Transport & Fuel","amount":100000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp169442071","date":"2026-03-13","description":"Kopi Klan","category":"Dining & Food","amount":135300,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp953842071","date":"2026-03-14","description":"Halaman Bhumi","category":"Dining & Food","amount":68000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp401342071","date":"2026-03-15","description":"Token Listrik","category":"Mobile Data & Internet","amount":104100,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp806942071","date":"2026-03-15","description":"TUKU","category":"Dining & Food","amount":300000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp873382071","date":"2026-03-16","description":"M","category":"Other Income","amount":300000,"type":"income","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp812092071","date":"2026-03-16","description":"THR Kantor","category":"Other Income","amount":1500000,"type":"income","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp388472071","date":"2026-03-16","description":"13 & 16 E Tol","category":"Transport & Fuel","amount":400000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp607992071","date":"2026-03-16","description":"THR Syifa","category":"Ritual Penggandaan Rezeki","amount":1500000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp277442071","date":"2026-03-18","description":"IPL & Iuran","category":"Subscriptions","amount":560000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp158242071","date":"2026-03-18","description":"Token Listrik","category":"Mobile Data & Internet","amount":104100,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp892892071","date":"2026-03-18","description":"Parcel Satpam","category":"Ritual Penggandaan Rezeki","amount":252900,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp101562071","date":"2026-03-19","description":"Apple Music (Dana Topup)","category":"Subscriptions","amount":100000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp526432071","date":"2026-03-19","description":"Terea","category":"Dining & Food","amount":70000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp168332071","date":"2026-03-19","description":"Maranggi Sari Asih","category":"Groceries","amount":78000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp732512071","date":"2026-03-20","description":"Ade Azhar","category":"Ritual Penggandaan Rezeki","amount":600000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp711732071","date":"2026-03-20","description":"Bensin","category":"Transport & Fuel","amount":200000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp788792071","date":"2026-03-20","description":"Gorengan","category":"Groceries","amount":50000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp718262071","date":"2026-03-21","description":"THR Ayah Ibu","category":"Other Income","amount":1000000,"type":"income","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp381192071","date":"2026-03-21","description":"THR","category":"Other Income","amount":425000,"type":"income","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp120172071","date":"2026-03-21","description":"Pulsa XL","category":"Mobile Data & Internet","amount":51099,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp993912071","date":"2026-03-21","description":"Pulsa XL","category":"Mobile Data & Internet","amount":51099,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp523482071","date":"2026-03-21","description":"THR ade","category":"Ritual Penggandaan Rezeki","amount":200000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp984082071","date":"2026-03-23","description":"Kawan Baru","category":"Dining & Food","amount":80400,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp939642071","date":"2026-03-23","description":"Parcel Susulan","category":"Shopping","amount":97100,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp784692071","date":"2026-03-23","description":"Parcel Susulan","category":"Shopping","amount":38000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp740662071","date":"2026-03-23","description":"Halaman Bhumi","category":"Dining & Food","amount":30000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp690642071","date":"2026-03-23","description":"Plutos Coffee","category":"Dining & Food","amount":152900,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp806922071","date":"2026-03-23","description":"Topup Steam Wallet","category":"Hobbies","amount":120659,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp964212071","date":"2026-03-23","description":"Terea","category":"Dining & Food","amount":72000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp577522071","date":"2026-03-24","description":"Sedari Coffee","category":"Dining & Food","amount":55000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp805812071","date":"2026-03-25","description":"Pompa Air Kolam","category":"Shopping","amount":252555,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp996112071","date":"2026-03-25","description":"Steam Wallet","category":"Hobbies","amount":58523,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp468682071","date":"2026-03-25","description":"Makan (Gopay Topup)","category":"Groceries","amount":36000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp342402071","date":"2026-03-25","description":"Kopi Klan","category":"Dining & Food","amount":52800,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp682492071","date":"2026-03-25","description":"Bensin","category":"Transport & Fuel","amount":340000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp953692071","date":"2026-03-27","description":"Gocar (Gopay Topup)","category":"Transport & Fuel","amount":83000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp986462071","date":"2026-03-27","description":"Servis Mobil","category":"Transport & Fuel","amount":1735925,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp884332071","date":"2026-03-27","description":"Terea","category":"Transport & Fuel","amount":70000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp541452071","date":"2026-03-27","description":"Gyu Kaku","category":"Groceries","amount":710424,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp458722071","date":"2026-03-28","description":"E Tol","category":"Transport & Fuel","amount":350000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp242472071","date":"2026-03-29","description":"Jajan Pasar Bersih Sentul","category":"Dining & Food","amount":40000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp431152071","date":"2026-03-29","description":"Token Listrik","category":"Mobile Data & Internet","amount":104100,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp228362071","date":"2026-03-30","description":"Point Coffee","category":"Dining & Food","amount":25000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp983872170","date":"2026-04-01","description":"M","category":"Other Income","amount":300000,"type":"income","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp656112170","date":"2026-04-01","description":"Roko Terea","category":"Dining & Food","amount":60000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp247112170","date":"2026-04-01","description":"Month Salary","category":"Monthly Salary","amount":4400000,"type":"income","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp221112170","date":"2026-04-01","description":"Kopi, Rokok","category":"Dining & Food","amount":70000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp385962170","date":"2026-04-01","description":"Soto Ayam Pomad","category":"Groceries","amount":29000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp549102170","date":"2026-04-02","description":"Racun Rumput","category":"Shopping","amount":16630,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp807522170","date":"2026-04-02","description":"Token Listrik","category":"Mobile Data & Internet","amount":501750,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp422382170","date":"2026-04-03","description":"Jajan Pagi Tukang","category":"Dining & Food","amount":47500,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp951772170","date":"2026-04-03","description":"Terea","category":"Dining & Food","amount":72000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp807462170","date":"2026-04-03","description":"Phone Holder","category":"Shopping","amount":96456,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp240822170","date":"2026-04-03","description":"Car Freshner","category":"Shopping","amount":34958,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp655772170","date":"2026-04-04","description":"EM4","category":"Shopping","amount":24500,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp781312170","date":"2026-04-04","description":"Syifa","category":"Ritual Penggandaan Rezeki","amount":3000000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp754322170","date":"2026-04-04","description":"Twenty Two Coffee Bellanova","category":"Dining & Food","amount":194700,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp745902170","date":"2026-04-04","description":"Penutup Kca Mobil","category":"Shopping","amount":34660,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp432282170","date":"2026-04-04","description":"Intis","category":"Dining & Food","amount":121000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp700472281","date":"2026-04-05","description":"Month Salary","category":"Monthly Salary","amount":4500000,"type":"income","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp124042170","date":"2026-04-06","description":"Arisan","category":"Savings","amount":200000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp599262170","date":"2026-04-06","description":"Brezza","category":"Dining & Food","amount":35000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp443372170","date":"2026-04-06","description":"E Tol","category":"Transport & Fuel","amount":300000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp710192170","date":"2026-04-07","description":"M","category":"Other Income","amount":548200,"type":"income","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp298192170","date":"2026-04-07","description":"Terea+Maizena","category":"Dining & Food","amount":37500,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp457482170","date":"2026-04-08","description":"Kopi Indomie Warung","category":"Dining & Food","amount":15000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp434712170","date":"2026-04-08","description":"Indihome","category":"Mobile Data & Internet","amount":366637,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp427892170","date":"2026-04-08","description":"IPL","category":"Subscriptions","amount":180000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp697102170","date":"2026-04-09","description":"Terea","category":"Dining & Food","amount":72000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp494282170","date":"2026-04-09","description":"Glaco Coating Glass","category":"Shopping","amount":251730,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp483182170","date":"2026-04-10","description":"Baju","category":"Shopping","amount":145195,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp394412170","date":"2026-04-11","description":"Kouta XL","category":"Mobile Data & Internet","amount":80000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp361202171","date":"2026-04-11","description":"Nasi Goreng","category":"Groceries","amount":30000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp442892171","date":"2026-04-11","description":"E Tol","category":"Transport & Fuel","amount":300000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp893822171","date":"2026-04-11","description":"Blackurant","category":"Dining & Food","amount":66000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp369572170","date":"2026-04-12","description":"Timun Mas Coffee Lab","category":"Dining & Food","amount":143000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp391802171","date":"2026-04-12","description":"Makanan Aceh Sentul","category":"Groceries","amount":52000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp696632171","date":"2026-04-12","description":"Terea","category":"Dining & Food","amount":108000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp462032171","date":"2026-04-12","description":"Tukang","category":"Ritual Penggandaan Rezeki","amount":50000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp991912170","date":"2026-04-13","description":"M","category":"Other Income","amount":300000,"type":"income","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp313132171","date":"2026-04-13","description":"Topup Shopee Pay","category":"Shopping","amount":50000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp943842171","date":"2026-04-13","description":"Microfiber Lap + Dryer","category":"Shopping","amount":101100,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp223802171","date":"2026-04-13","description":"Kopi Warung","category":"Dining & Food","amount":5000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp751002171","date":"2026-04-13","description":"Cukur","category":"Self Care","amount":55000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp549622171","date":"2026-04-15","description":"Gopay Topup (Terea)","category":"Dining & Food","amount":90000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp513012171","date":"2026-04-15","description":"Token Listrik","category":"Mobile Data & Internet","amount":500000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp636932171","date":"2026-04-15","description":"Piscok + Es Jeruk 16 Apr","category":"Dining & Food","amount":15000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp421372171","date":"2026-04-15","description":"E Tol","category":"Transport & Fuel","amount":300000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp204352171","date":"2026-04-16","description":"IEM Kinera Wyvern","category":"Shopping","amount":273516,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp354642171","date":"2026-04-17","description":"Gopay Topup (Terea)","category":"Dining & Food","amount":55000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp767612171","date":"2026-04-19","description":"Gopay Topup (ES Teh)","category":"Dining & Food","amount":48000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp880432170","date":"2026-04-20","description":"M","category":"Other Income","amount":300000,"type":"income","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp902322171","date":"2026-04-20","description":"Magnesium","category":"Self Care","amount":73778,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp657102171","date":"2026-04-20","description":"Apple Cloud Musid (Dana)","category":"Subscriptions","amount":110000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp984662171","date":"2026-04-20","description":"Claude (Dana Topup)","category":"Subscriptions","amount":350000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp999132171","date":"2026-04-22","description":"Baju Olahraga Syifa","category":"Ritual Penggandaan Rezeki","amount":219720,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp461742171","date":"2026-04-23","description":"Ade Azhar","category":"Ritual Penggandaan Rezeki","amount":100000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp385912171","date":"2026-04-24","description":"Rahang Tuna","category":"Groceries","amount":90000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp839542171","date":"2026-04-25","description":"E Money (21&25 April)","category":"Transport & Fuel","amount":550000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp797222171","date":"2026-04-25","description":"Bonex","category":"Groceries","amount":53000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp842832171","date":"2026-04-26","description":"Warisan Kopi","category":"Dining & Food","amount":60000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp262002171","date":"2026-04-26","description":"PLN Mobile","category":"Mobile Data & Internet","amount":501750,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp269352171","date":"2026-04-26","description":"Naturehike Dry Bag","category":"Shopping","amount":182851,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp769752171","date":"2026-04-26","description":"Coca Cola","category":"Dining & Food","amount":15000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp385582170","date":"2026-04-27","description":"M","category":"Other Income","amount":300000,"type":"income","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp183452171","date":"2026-04-28","description":"Beras","category":"Groceries","amount":74724,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp610642170","date":"2026-04-29","description":"S","category":"Other Income","amount":150000,"type":"income","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp619082171","date":"2026-04-29","description":"1800 + Black Label","category":"Dining & Food","amount":1100000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp465662171","date":"2026-04-29","description":"Topron (ShopeePay Topup)","category":"Shopping","amount":36000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp175332170","date":"2026-04-30","description":"S","category":"Other Income","amount":300000,"type":"income","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp432132281","date":"2026-05-02","description":"ETol","category":"Transport & Fuel","amount":300000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp545102281","date":"2026-05-02","description":"Tyo + Tomyum","category":"Ritual Penggandaan Rezeki","amount":300000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp592382281","date":"2026-05-02","description":"Ayam Taliwang","category":"Groceries","amount":103000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp544832281","date":"2026-05-03","description":"COD MW Remastered","category":"Hobbies","amount":19200,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp822582281","date":"2026-05-05","description":"Pull Up Bar (Shopeepay)","category":"Shopping","amount":120000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp994582281","date":"2026-05-05","description":"Syifa","category":"Ritual Penggandaan Rezeki","amount":3000000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp432602281","date":"2026-05-05","description":"Kouta XL","category":"Mobile Data & Internet","amount":80000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp223032281","date":"2026-05-05","description":"Nasi Goreng (Gopay Topup)","category":"Groceries","amount":66000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp608632281","date":"2026-05-05","description":"Admin Jenius","category":"Subscriptions","amount":20000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp428822281","date":"2026-05-06","description":"Extra Income","category":"Other Income","amount":300000,"type":"income","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp669962281","date":"2026-05-07","description":"Token Listrik","category":"Mobile Data & Internet","amount":501750,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp104372281","date":"2026-05-07","description":"IPL","category":"Subscriptions","amount":180000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp511582281","date":"2026-05-07","description":"Makan","category":"Groceries","amount":20000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp343912281","date":"2026-05-08","description":"Aris","category":"Other Income","amount":2800000,"type":"income","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp197052281","date":"2026-05-08","description":"SPPD","category":"Other Income","amount":100000,"type":"income","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp808022281","date":"2026-05-08","description":"Cuci Mobil","category":"Transport & Fuel","amount":40000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp194222281","date":"2026-05-09","description":"Insole","category":"Shopping","amount":128949,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp714552281","date":"2026-05-10","description":"Ud Djaya","category":"Dining & Food","amount":51700,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp595002281","date":"2026-05-10","description":"Pisang Pempek, Tekwan","category":"Groceries","amount":85000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp996292281","date":"2026-05-10","description":"Cukur","category":"Self Care","amount":40000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp798422281","date":"2026-05-10","description":"ETol","category":"Transport & Fuel","amount":200000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp653962281","date":"2026-05-12","description":"Extra Income","category":"Other Income","amount":300000,"type":"income","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp169092281","date":"2026-05-13","description":"Makan Warung","category":"Dining & Food","amount":19000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp594002281","date":"2026-05-13","description":"Lightroom","category":"Subscriptions","amount":30000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp317142281","date":"2026-05-13","description":"Beard Papa","category":"Groceries","amount":48000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp201882281","date":"2026-05-16","description":"Ikan Parung Iko","category":"Hobbies","amount":480000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp726162281","date":"2026-05-17","description":"Japanese Food","category":"Groceries","amount":94000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp890462281","date":"2026-05-17","description":"ETol","category":"Transport & Fuel","amount":750000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp490282281","date":"2026-05-18","description":"Kouta XL","category":"Mobile Data & Internet","amount":80000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp634552281","date":"2026-05-18","description":"Internet","category":"Mobile Data & Internet","amount":367800,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp689592281","date":"2026-05-19","description":"Extra Income","category":"Other Income","amount":300000,"type":"income","wallet":"Cash","notes":"Imported from MAY"}];
...
function UserApp({user,onLogout}){
  ...
  useEffect(()=>{loadData(user.id).then(d=>{const applied=applyRecurring(d);setData(applied);setDarkMode(!!applied.darkMode);});},[]);
