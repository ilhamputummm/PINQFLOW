...
// ─── DASHBOARD ────────────────────────────────────────────────────────────────
function SwipeBalanceCard({income,expense,saving,savePct,data,selectedMonth,C,hidden}){
  ...
  const target=data.settings?.savingsTarget??60;

  // YTD calculations
  const now=new Date();
  const year=now.getFullYear();
  const ytdTxs=data.transactions.filter(t=>t.date>=`${year}-01-01`);
  ...
  const ytdNet=ytdInc-ytdExp;
  const ytdRate=ytdInc>0?(ytdNet/ytdInc)*100:0;

  // Monthly balance (all wallets all time)
  const totalBal=(data.wallets||DEFAULT_WALLETS).reduce((s,w)=>s+walletBal(w,data.transactions),0);

  const goTo=(idx)=>{setSlide(Math.max(0,Math.min(1,idx)));};

  const onPointerDown=(e)=>{setDragging(true);setStartX(e.clientX||e.touches?.[0]?.clientX||0);setDragX(0);};
  const onPointerMove=(e)=>{
    if(!dragging)return;
    const x=(e.clientX||e.touches?.[0]?.clientX||0)-startX;
    setDragX(x);
    if(trackRef.current)trackRef.current.style.transform=`translateX(calc(${-slide*100}% + ${x}px))`;
  };
  const onPointerUp=()=>{
    if(!dragging)return;
    setDragging(false);
    if(dragX<-50)goTo(slide+1);
    else if(dragX>50)goTo(slide-1);
    else goTo(slide);
    setDragX(0);
  };
  ...
  const blurStyle=hidden?{filter:"blur(10px)",userSelect:"none"}:{};
  const bg0=isDark?"linear-gradient(135deg,#1a2540,#0f1a32)":"linear-gradient(135deg,#1A2151,#3D4E7A)";
  const bg1=isDark?"linear-gradient(135deg,#0f2027,#1a3040)":"linear-gradient(135deg,#0f2027,#203a43)";

  return(
    <div style={{marginBottom:10}}>
      <div style={{borderRadius:20,overflow:"hidden",position:"relative",cursor:"grab",touchAction:"pan-y"}}
        onMouseDown={onPointerDown} onMouseMove={onPointerMove} onMouseUp={onPointerUp} onMouseLeave={onPointerUp}
        onTouchStart={onPointerDown} onTouchMove={onPointerMove} onTouchEnd={onPointerUp}>
        <div ref={trackRef} style={{display:"flex",willChange:"transform",transition:dragging?"none":"transform 0.45s cubic-bezier(0.25,0.46,0.45,0.94)"}}>

          {/* CARD 0 — Monthly */}
          <div style={{minWidth:"100%",padding:"20px 22px",background:bg0,position:"relative",overflow:"hidden",boxSizing:"border-box"}}>
            ...
            <div style={{display:"inline-flex",alignItems:"center",gap:5,background:"rgba(96,165,250,0.15)",borderRadius:20,padding:"3px 10px",marginBottom:10}}>
              <div style={{width:6,height:6,borderRadius:"50%",background:"#60A5FA"}}/>
              <span style={{fontSize:10,fontWeight:700,color:"rgba(255,255,255,0.7)",letterSpacing:0.5}}>THIS MONTH · {mkLblS(selectedMonth).toUpperCase()}</span>
            </div>
            <div style={{fontSize:11,color:"rgba(255,255,255,0.55)",fontWeight:700,letterSpacing:1,marginBottom:5}}>NET THIS MONTH</div>
            <div style={{fontFamily:MONO,fontSize:28,fontWeight:900,color:saving>=0?"#34D399":"#F87171",letterSpacing:-0.5,marginBottom:14,...blurStyle}}>{rp(saving)}</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
              <div style={{background:"rgba(52,211,153,0.15)",borderRadius:12,padding:"10px 12px"}}>
                <div style={{fontSize:10,color:"rgba(255,255,255,0.5)",fontWeight:600,marginBottom:3}}>Income</div>
                <span style={{fontFamily:MONO,fontSize:14,fontWeight:800,color:"#34D399",...blurStyle}}>{rp(income)}</span>
              </div>
              <div style={{background:"rgba(248,113,113,0.15)",borderRadius:12,padding:"10px 12px"}}>
                <div style={{fontSize:10,color:"rgba(255,255,255,0.5)",fontWeight:600,marginBottom:3}}>Expense</div>
                <span style={{fontFamily:MONO,fontSize:14,fontWeight:800,color:"#F87171",...blurStyle}}>{rp(expense)}</span>
              </div>
            </div>
            <div style={{height:1,background:"rgba(255,255,255,0.1)",marginBottom:14}}/>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div>
                <div style={{fontSize:10,color:"rgba(255,255,255,0.5)",fontWeight:600,marginBottom:3}}>ALL WALLETS BALANCE</div>
                <span style={{fontFamily:MONO,fontSize:18,fontWeight:800,color:"#fff",...blurStyle}}>{rp(totalBal)}</span>
              </div>
              <div style={{textAlign:"right"}}>
                <div style={{fontSize:10,color:"rgba(255,255,255,0.45)",marginBottom:2}}>Saving Rate</div>
                <div style={{fontFamily:MONO,fontSize:22,fontWeight:900,color:savePct>=target?"#8B7FE8":"#F87171",lineHeight:1}}>{savePct.toFixed(0)}%</div>
                <div style={{fontSize:10,color:"rgba(255,255,255,0.35)",marginTop:2}}>Target {target}%</div>
              </div>
            </div>
          </div>

          {/* CARD 1 — YTD */}
          <div style={{minWidth:"100%",padding:"20px 22px",background:bg1,position:"relative",overflow:"hidden",boxSizing:"border-box"}}>
            <div style={{position:"absolute",right:-20,top:-20,width:120,height:120,borderRadius:"50%",background:"rgba(255,255,255,0.03)"}}/>
            <div style={{position:"absolute",left:20,bottom:-30,width:90,height:90,borderRadius:"50%",background:"rgba(255,255,255,0.02)"}}/>
            <div style={{display:"inline-flex",alignItems:"center",gap:5,background:"rgba(52,211,153,0.15)",borderRadius:20,padding:"3px 10px",marginBottom:10}}>
              <div style={{width:6,height:6,borderRadius:"50%",background:"#34D399"}}/>
              <span style={{fontSize:10,fontWeight:700,color:"rgba(255,255,255,0.7)",letterSpacing:0.5}}>YEAR TO DATE · {year}</span>
            </div>
            <div style={{fontSize:11,color:"rgba(255,255,255,0.55)",fontWeight:700,letterSpacing:1,marginBottom:5}}>NET SAVINGS YTD</div>
            <div style={{fontFamily:MONO,fontSize:28,fontWeight:900,color:ytdNet>=0?"#34D399":"#F87171",letterSpacing:-0.5,marginBottom:14,...blurStyle}}>{rp(ytdNet)}</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
              <div style={{background:"rgba(52,211,153,0.15)",borderRadius:12,padding:"10px 12px"}}>
                <div style={{fontSize:10,color:"rgba(255,255,255,0.5)",fontWeight:600,marginBottom:3}}>Total Income</div>
                <span style={{fontFamily:MONO,fontSize:14,fontWeight:800,color:"#34D399",...blurStyle}}>{rp(ytdInc)}</span>
              </div>
              <div style={{background:"rgba(248,113,113,0.15)",borderRadius:12,padding:"10px 12px"}}>
                <div style={{fontSize:10,color:"rgba(255,255,255,0.5)",fontWeight:600,marginBottom:3}}>Total Expense</div>
                <span style={{fontFamily:MONO,fontSize:14,fontWeight:800,color:"#F87171",...blurStyle}}>{rp(ytdExp)}</span>
              </div>
            </div>
            <div style={{height:1,background:"rgba(255,255,255,0.1)",marginBottom:14}}/>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div>
                <div style={{fontSize:10,color:"rgba(255,255,255,0.5)",fontWeight:600,marginBottom:3}}>NET SAVINGS JAN – {MONTH_S[now.getMonth()].toUpperCase()}</div>
                <span style={{fontFamily:MONO,fontSize:18,fontWeight:800,color:ytdNet>=0?"#34D399":"#F87171",...blurStyle}}>{rp(ytdNet)}</span>
              </div>
              <div style={{textAlign:"right"}}>
                <div style={{fontSize:10,color:"rgba(255,255,255,0.45)",marginBottom:2}}>Avg Rate</div>
                <div style={{fontFamily:MONO,fontSize:22,fontWeight:900,color:ytdRate>=target?"#8B7FE8":"#F87171",lineHeight:1}}>{ytdRate.toFixed(0)}%</div>
                <div style={{fontSize:10,color:"rgba(255,255,255,0.35)",marginTop:2}}>Target {target}%</div>
              </div>
            </div>
          </div>
      ...
      <div style={{display:"flex",justifyContent:"center",gap:6,marginTop:10}}>
        {[0,1].map(i=>(
          <div key={i} onClick={()=>goTo(i)} style={{height:6,borderRadius:3,background:slide===i?C.navy:`${C.navy}25`,width:slide===i?20:6,transition:"all 0.35s cubic-bezier(0.25,0.46,0.45,0.94)",cursor:"pointer"}}/>
        ))}
      </div>
    </div>
  );
...
function Dashboard({txs,income,expense,saving,savePct,data,selectedMonth,hidden,setHidden}){
  const C=useTheme();
  ...
  const earlyAlerts=Object.entries(data.budgets||{}).map(([cat,bud])=>{const s=bycat[cat]||0;const p=bud>0?(s/bud)*100:0;return{cat,spent:s,bud,pct:p};}).filter(a=>a.pct>=70&&a.pct<100).sort((a,b)=>b.pct-a.pct).slice(0,3);
  const activeGoals=(data.goals||[]).filter(g=>g.saved<g.target).sort((a,b)=>{if(!a.deadline&&!b.deadline)return 0;if(!a.deadline)return 1;if(!b.deadline)return -1;return a.deadline.localeCompare(b.deadline);}).slice(0,2);
  const upcoming=(data.recurring||[]).filter(r=>{if(!r.active||r.frequency!=="monthly")return false;const diff=r.dayOfMonth-now.getDate();return diff>=0&&diff<=7;}).map(r=>({...r,daysLeft:r.dayOfMonth-now.getDate()})).sort((a,b)=>a.daysLeft-b.daysLeft);
  const isDark=C.bg===CD.bg;
  ...
  return(
    <div style={{padding:"12px 14px 0"}}>
      ...
      <SwipeBalanceCard income={income} expense={expense} saving={saving} savePct={savePct} data={data} selectedMonth={selectedMonth} C={C} hidden={hidden}/>
        ...
        <div style={{...card,padding:"14px 18px",marginBottom:10}}>
          ...
          {upcoming.map(r=>(
            <div key={r.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 0",borderBottom:`1px solid ${C.border}`}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                <div style={{width:34,height:34,borderRadius:10,background:`${C.coral}15`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14}}>{catIcon(r.category)}</div>
                <div>
                  <div style={{fontSize:13,fontWeight:600,color:C.navy}}>{r.description}</div>
                  <div style={{fontSize:11,color:r.daysLeft===0?C.coral:C.muted}}>{r.daysLeft===0?"Due today!":r.daysLeft===1?"Due tomorrow":`In ${r.daysLeft} days`}</div>
                </div>
              </div>
      ...
      {alerts.length>0&&(
        <div style={{marginBottom:10}}>
          ...
          {alerts.map(a=>(
            <div key={a.cat} style={{...cardSm,padding:"12px 16px",marginBottom:8,background:a.pct>=100?"linear-gradient(135deg,#7C3AED,#8B5CF6)":`${C.gold}15`,color:a.pct>=100?"#fff":C.navy,border:a.pct>=100?"none":`1px solid ${C.gold}30`,position:"relative",overflow:"hidden"}}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <div>
                  ...
                  <div style={{fontSize:11,opacity:.8,color:a.pct>=100?"rgba(255,255,255,0.8)":C.muted,fontFamily:MONO,marginTop:2}}>{rp(a.spent)} / {rp(a.bud)}</div>
                </div>
          ...
          {activeGoals.map(g=>{
            ...
            return(
              <div key={g.id} style={{marginBottom:12}}>
                ...
                <div style={{fontSize:10,color:C.muted,marginTop:3}}>{rp(g.saved)} / {rp(g.target)} · {rp(left)} to go</div>
              </div>
            );
          })}
        </div>
      ...
      <div style={{...card,padding:"16px 18px",marginBottom:80}}>
        ...
        {[...txs].reverse().slice(0,6).map((t,i,arr)=>(
          <div key={t.id} style={{display:"flex",gap:12,alignItems:"center",paddingBottom:i<arr.length-1?12:0,marginBottom:i<arr.length-1?12:0,borderBottom:i<arr.length-1?`1px solid ${C.border}`:"none"}}>
            <div style={{width:40,height:40,borderRadius:12,flexShrink:0,background:t.type==="income"?`${C.mint}20`:t.type==="transfer"?`${C.lavender}18`:`${CAT_COLOR[t.category]||C.coral}15`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>
...
function Transactions({txs,data,updateData}){
  ...
  const [editAnchorId,setEditAnchorId]=useState(null);
  const startEdit=(t)=>{
    setEditTx(t);
    setEditAnchorId(t.id);
    setEditForm({description:t.description,amount:t.amount.toLocaleString("id-ID"),date:t.date,category:t.category,wallet:t.wallet||"Cash",notes:t.notes||""});
  };
  const saveEdit=()=>{
    ...
    setEditTx(null);setEditAnchorId(null);
  };
  ...
  const cats=editTx?.type==="income"?INCOME_CATS:EXPENSE_CATS;
  return(
    <div style={{padding:"16px 14px 100px"}}>
      ...
      {list.map((t)=>(
        ...
        <div style={{...cardSm,padding:"13px 14px",marginBottom:8,display:"flex",gap:12,alignItems:"center"}}>
        ...
        {/* Inline edit form — appears directly below the tapped transaction */}
        {editAnchorId===t.id&&editTx&&(
          <div style={{...card,borderRadius:20,padding:"18px 18px 20px",marginBottom:8,border:`2px solid ${C.mint}40`,animation:"fadeUp .2s ease"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
              <div style={{fontSize:14,fontWeight:800,color:C.navy}}>✏️ Edit Transaction</div>
              <button onClick={()=>{setEditTx(null);setEditAnchorId(null);}} style={{background:"none",border:"none",fontSize:18,color:C.muted,cursor:"pointer",lineHeight:1}}>✕</button>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:11}}>
              <div><div style={{fontSize:10,color:C.muted,fontWeight:700,letterSpacing:1,marginBottom:4}}>DATE</div><DatePicker value={editForm.date} onChange={v=>setEditForm(f=>({...f,date:v}))}/></div>
              <div><div style={{fontSize:10,color:C.muted,fontWeight:700,letterSpacing:1,marginBottom:4}}>DESCRIPTION</div><input type="text" value={editForm.description} onChange={e=>setEditForm(f=>({...f,description:e.target.value}))} style={inpE}/></div>
              {editTx.type!=="transfer"&&(
                <div><div style={{fontSize:10,color:C.muted,fontWeight:700,letterSpacing:1,marginBottom:4}}>CATEGORY</div>
                  <CustomSelect value={editForm.category} onChange={v=>setEditForm(f=>({...f,category:v}))} options={cats.map(c=>({value:c,label:c,icon:catIcon(c)}))}/>
                </div>
              )}
              <div><div style={{fontSize:10,color:C.muted,fontWeight:700,letterSpacing:1,marginBottom:4}}>WALLET</div>
                <CustomSelect value={editForm.wallet} onChange={v=>setEditForm(f=>({...f,wallet:v}))} options={wallets.map(w=>({value:w,label:w,icon:wEmoji(w)}))}/>
              </div>
              <div><div style={{fontSize:10,color:C.muted,fontWeight:700,letterSpacing:1,marginBottom:4}}>AMOUNT (IDR)</div>
                <input type="text" inputMode="numeric" value={editForm.amount} onChange={e=>setEditForm(f=>({...f,amount:fmtE(e.target.value)}))} style={{...inpE,fontFamily:MONO,fontWeight:700,color:editTx.type==="income"?C.mintD:C.coral}}/>
              </div>
              <div><div style={{fontSize:10,color:C.muted,fontWeight:700,letterSpacing:1,marginBottom:4}}>NOTES (optional)</div>
                <input type="text" placeholder="Optional note..." value={editForm.notes||""} onChange={e=>setEditForm(f=>({...f,notes:e.target.value}))} style={inpE}/>
              </div>
            </div>
            <div style={{display:"flex",gap:9,marginTop:14}}>
              <button onClick={()=>{setEditTx(null);setEditAnchorId(null);}} style={{flex:1,background:C.border,border:"none",borderRadius:14,padding:"13px 0",color:C.muted,fontSize:13,fontWeight:700,cursor:"pointer"}}>Cancel</button>
              <button onClick={saveEdit} style={{flex:2,background:`linear-gradient(135deg,${C.mint},${C.mintD})`,border:"none",borderRadius:14,padding:"13px 0",color:"#fff",fontSize:13,fontWeight:800,cursor:"pointer",boxShadow:`0 6px 20px ${C.mint}40`}}>Save Changes</button>
            </div>
          </div>
        )}
        </React.Fragment>
            ...
            <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:16}}>
              <ResponsiveContainer width={130} height={130}>
                <PieChart>
                  ...
                  <Tooltip formatter={v=>rp(v)} contentStyle={{background:C.bgCard,border:`1px solid ${C.border}`,borderRadius:10,fontSize:11}}/>
                </PieChart>
              </ResponsiveContainer>
        ...
        {[{l:"💰 Savings Target",v:savTarget,c:C.mintD,set:(v)=>{setSavTarget(v);setExpMax(e=>Math.min(e,100-v));}},{l:"💸 Max Expense",v:expMax,c:C.coral,set:(v)=>{setExpMax(v);setSavTarget(s=>Math.min(s,100-v));}}].map(({l,v,c,set})=>(
          <div key={l} style={{marginBottom:16}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:7}}>
              ...
              <span style={{fontFamily:MONO,fontSize:13,fontWeight:800,color:c}}>{v}%{income>0&&<span style={{fontWeight:500,color:C.muted}}> · {rp(income*v/100)}</span>}</span>
            </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            {TEMPLATES.map(t=>(
              <button key={t.name} onClick={()=>{setForm({name:t.name,target:t.target.toLocaleString("id-ID"),saved:"",deadline:""});setAdding(true);}}
                ...
                <div style={{fontFamily:MONO,fontSize:11,fontWeight:700,color:C.mintD,marginTop:4}}>{rp(t.target)}</div>
              </button>
            ))}
          </div>
        </div>
        ...
        return(
          <div key={g.id} style={{...card,padding:"18px 18px",marginBottom:12,border:done?`1.5px solid ${C.mint}`:"none"}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",marginTop:6}}>
              ...
              {!done&&<span style={{fontSize:11,color:C.muted}}>{rp(left)} to go</span>}
            </div>
            {!done&&(
              <div style={{marginTop:12}}>
                <div style={{display:"flex",gap:6,marginBottom:6}}>
                  {[50000,100000,500000].map(a=>(
                    <button key={a} onClick={()=>addSavings(g.id,a)} style={{flex:1,background:`${C.mint}15`,border:`1px solid ${C.mint}30`,borderRadius:10,padding:"7px 0",color:C.mintD,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:MONO}}>+{rp(a)}</button>
                  ))}
                </div>
  ...
  return(
    <div style={{padding:16}}>
      ...
      <div style={{...card,padding:"14px 18px",marginBottom:16,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        ...
        <button onClick={()=>{const nd=!darkMode;setDarkMode(nd);if(data)updateData({...data,darkMode:nd});}} style={{width:52,height:28,borderRadius:14,border:"none",cursor:"pointer",background:darkMode?C.lavender:C.border,position:"relative",transition:"background .3s",flexShrink:0}}>
...
// ─── PUTU IMPORT DATA (Jan–May 2026) ──────────────────────────────────────────
const PUTU_IMPORT_DATA = [{"id":"imp999873278","date":"2026-01-01","description":"S 18 December","category":"Other Income","amount":1000000,"type":"income","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp207523278","date":"2026-01-01","description":"Tanmu Coffee","category":"Dining & Food","amount":78000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp683723278","date":"2026-01-01","description":"S 19 December","category":"Other Income","amount":400000,"type":"income","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp577113278","date":"2026-01-01","description":"Baso Holly","category":"Groceries","amount":30000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp109983278","date":"2026-01-01","description":"SPPD 19 December","category":"Other Income","amount":1677000,"type":"income","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp839073278","date":"2026-01-01","description":"Terea","category":"Dining & Food","amount":70000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp268483278","date":"2026-01-01","description":"Akhir Taun O","category":"Other Income","amount":300000,"type":"income","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp655693278","date":"2026-01-01","description":"Syifa","category":"Ritual Penggandaan Rezeki","amount":3000000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp693843278","date":"2026-01-01","description":"Monyh Salary","category":"Other Income","amount":4800000,"type":"income","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp401253278","date":"2026-01-01","description":"M","category":"Other Income","amount":500000,"type":"income","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp561953278","date":"2026-01-02","description":"E Tol","category":"Transport & Fuel","amount":300000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp697243278","date":"2026-01-02","description":"Coffee Companion","category":"Dining & Food","amount":95000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp363223278","date":"2026-01-02","description":"Bensin Mobil","category":"Transport & Fuel","amount":280000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp615233278","date":"2026-01-03","description":"Token Listrik Sentul","category":"Mobile Data & Internet","amount":306700,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp958073278","date":"2026-01-03","description":"Bensin Motor","category":"Transport & Fuel","amount":90000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp579183279","date":"2026-01-03","description":"Duren Cimahpar","category":"Groceries","amount":130000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp965393278","date":"2026-01-04","description":"Uang Kebersamaan Star House","category":"Dining & Food","amount":110000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp596463278","date":"2026-01-04","description":"Indomaret","category":"Dining & Food","amount":103000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp462023278","date":"2026-01-05","description":"M","category":"Other Income","amount":500000,"type":"income","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp275323278","date":"2026-01-05","description":"M","category":"Other Income","amount":268000,"type":"income","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp357973279","date":"2026-01-06","description":"Terea","category":"Dining & Food","amount":70000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp155813279","date":"2026-01-06","description":"Apple Subscribe (DANA)","category":"Subscriptions","amount":200000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp457103279","date":"2026-01-06","description":"Tanaman Alocasia","category":"Hobbies","amount":19000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp660323279","date":"2026-01-07","description":"Kopi Warung","category":"Dining & Food","amount":12000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp948443279","date":"2026-01-07","description":"Makan Warung","category":"Groceries","amount":15000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp586893279","date":"2026-01-07","description":"Pot Tanaman","category":"Hobbies","amount":21195,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp348713279","date":"2026-01-08","description":"Case iPhone Spigen","category":"Shopping","amount":315944,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp329443279","date":"2026-01-08","description":"Media Tanam","category":"Hobbies","amount":27800,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp912463279","date":"2026-01-08","description":"Sekop","category":"Hobbies","amount":13060,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp703243279","date":"2026-01-08","description":"Monstera Monthai","category":"Hobbies","amount":94400,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp542463279","date":"2026-01-08","description":"WiFi Indihome","category":"Mobile Data & Internet","amount":143824,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp458733279","date":"2026-01-09","description":"Makan Kopi Warung","category":"Groceries","amount":21000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp761733279","date":"2026-01-09","description":"Semprotan Tanaman","category":"Hobbies","amount":55800,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp498493279","date":"2026-01-09","description":"Kepala Charger UGreen","category":"Shopping","amount":166469,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp275463279","date":"2026-01-09","description":"Shell","category":"Transport & Fuel","amount":329819,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp291103279","date":"2026-01-10","description":"Terea","category":"Dining & Food","amount":70000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp557893279","date":"2026-01-11","description":"Microsoft","category":"Shopping","amount":130000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp351773279","date":"2026-01-11","description":"Gopay TopUp (Newport)","category":"Subscriptions","amount":116000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp495383279","date":"2026-01-11","description":"P2P Aquarium","category":"Hobbies","amount":20000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp532943279","date":"2026-01-11","description":"E Tol","category":"Transport & Fuel","amount":300000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp245143278","date":"2026-01-12","description":"M","category":"Other Income","amount":500000,"type":"income","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp172703279","date":"2026-01-12","description":"Kopi Warung","category":"Dining & Food","amount":4000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp431073279","date":"2026-01-12","description":"Taneman Veitchii & Alocasia","category":"Hobbies","amount":130234,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp318363279","date":"2026-01-12","description":"Makan Warung","category":"Groceries","amount":15000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp424653279","date":"2026-01-12","description":"Terea","category":"Dining & Food","amount":35000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp926813279","date":"2026-01-13","description":"Kopi Warung","category":"Dining & Food","amount":4000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp497073279","date":"2026-01-14","description":"Terea","category":"Dining & Food","amount":70000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp565613279","date":"2026-01-14","description":"Kopi Warung","category":"Dining & Food","amount":4000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp691913278","date":"2026-01-15","description":"S","category":"Other Income","amount":600000,"type":"income","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp339663279","date":"2026-01-15","description":"Kopi Warung","category":"Dining & Food","amount":8000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp713473279","date":"2026-01-15","description":"Bensin","category":"Transport & Fuel","amount":350000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp212073279","date":"2026-01-15","description":"E Tol","category":"Transport & Fuel","amount":198500,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp499573279","date":"2026-01-16","description":"Tambal Ban","category":"Transport & Fuel","amount":25000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp302183279","date":"2026-01-16","description":"Dimitree","category":"Dining & Food","amount":169400,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp125393279","date":"2026-01-16","description":"Kouta Telkomsel","category":"Mobile Data & Internet","amount":90000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp326943278","date":"2026-01-17","description":"F","category":"Other Income","amount":330000,"type":"income","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp820683279","date":"2026-01-17","description":"Pasang Meja","category":"Ritual Penggandaan Rezeki","amount":100000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp953643279","date":"2026-01-17","description":"Irman","category":"Ritual Penggandaan Rezeki","amount":100000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp558443279","date":"2026-01-17","description":"Cukur","category":"Self Care","amount":40000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp763193279","date":"2026-01-17","description":"Alfamart","category":"Dining & Food","amount":108300,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp333723279","date":"2026-01-18","description":"Makan Pasar Bersih + Tuku","category":"Groceries","amount":114000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp287373279","date":"2026-01-18","description":"USB Type C Extension Cable","category":"Shopping","amount":127752,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp441313279","date":"2026-01-20","description":"M","category":"Other Income","amount":500000,"type":"income","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp487383279","date":"2026-01-20","description":"Kopi Roko Warung","category":"Dining & Food","amount":37000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp925693279","date":"2026-01-20","description":"Terea + Sabun","category":"Dining & Food","amount":75000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp373963279","date":"2026-01-21","description":"S","category":"Other Income","amount":200000,"type":"income","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp196613279","date":"2026-01-21","description":"Kopi Warung","category":"Dining & Food","amount":4000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp335383279","date":"2026-01-21","description":"Kunci L","category":"Shopping","amount":63350,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp725713279","date":"2026-01-23","description":"Terea","category":"Dining & Food","amount":90000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp627143279","date":"2026-01-23","description":"Game Uncharted","category":"Hobbies","amount":25900,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp205993279","date":"2026-01-23","description":"Tissue+Sayur","category":"Shopping","amount":35000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp598553279","date":"2026-01-24","description":"Kerupuk","category":"Groceries","amount":10000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp668693279","date":"2026-01-24","description":"Bensin BP","category":"Transport & Fuel","amount":320000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp936813279","date":"2026-01-24","description":"Kouta XL","category":"Mobile Data & Internet","amount":80000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp893573279","date":"2026-01-25","description":"E Tol","category":"Transport & Fuel","amount":300000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp991943279","date":"2026-01-26","description":"Daging + Gas","category":"Groceries","amount":285000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp547993279","date":"2026-01-26","description":"Roko + Kopi Warung","category":"Dining & Food","amount":30000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp289283279","date":"2026-01-27","description":"M","category":"Other Income","amount":500000,"type":"income","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp444913279","date":"2026-01-27","description":"Makan + Kopi Warung","category":"Dining & Food","amount":25000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp194823279","date":"2026-01-27","description":"Terea Alfamart","category":"Dining & Food","amount":124000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp353403279","date":"2026-01-28","description":"Kopi Warung","category":"Dining & Food","amount":4000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp101943279","date":"2026-01-28","description":"FlashDisk 16Gb","category":"Shopping","amount":73400,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp371233279","date":"2026-01-28","description":"Topup Dana","category":"Subscriptions","amount":100000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp691933279","date":"2026-01-29","description":"Daging","category":"Groceries","amount":107600,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp327293279","date":"2026-01-29","description":"Testpen","category":"Shopping","amount":26900,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp863963279","date":"2026-01-30","description":"Kopi Roko  Warung","category":"Dining & Food","amount":37000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp423273279","date":"2026-01-30","description":"Terea","category":"Dining & Food","amount":71000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp184523279","date":"2026-01-30","description":"E Tol","category":"Transport & Fuel","amount":300000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp964453279","date":"2026-01-31","description":"Pindah Budget Ke Februari","category":"Savings","amount":420053,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp728163279","date":"2026-01-31","description":"Bensin","category":"Transport & Fuel","amount":310000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp388843279","date":"2026-01-31","description":"Listrik","category":"Mobile Data & Internet","amount":202600,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp330133279","date":"2026-01-31","description":"Bensin Vivo","category":"Transport & Fuel","amount":310000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp520613279","date":"2026-01-31","description":"Gopay TopUp (Kebab)","category":"Dining & Food","amount":81000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp100573279","date":"2026-01-31","description":"Admin, Parkir dll","category":"Shopping","amount":100000,"type":"expense","wallet":"Cash","notes":"Imported from JANUARY"},{"id":"imp573003394","date":"2026-02-01","description":"S & Arif (29&30 Januari)","category":"Other Income","amount":2300000,"type":"income","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp587743394","date":"2026-02-01","description":"Token Listrik","category":"Mobile Data & Internet","amount":203600,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp355583394","date":"2026-02-01","description":"Breza Coffee","category":"Dining & Food","amount":59000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp406663394","date":"2026-02-01","description":"Sisa Januari","category":"Other Income","amount":420053,"type":"income","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp936773394","date":"2026-02-01","description":"Terea","category":"Dining & Food","amount":71000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp653733394","date":"2026-02-02","description":"M","category":"Other Income","amount":500000,"type":"income","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp673233394","date":"2026-02-02","description":"Month Salary","category":"Monthly Salary","amount":4500000,"type":"income","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp420453394","date":"2026-02-02","description":"Sirloin HJK","category":"Groceries","amount":87000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp211393394","date":"2026-02-02","description":"Kopi Warung","category":"Dining & Food","amount":4000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp146873394","date":"2026-02-03","description":"E Tol","category":"Transport & Fuel","amount":300000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp491773394","date":"2026-02-03","description":"Makan Warung","category":"Groceries","amount":17000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp233583394","date":"2026-02-04","description":"Kopi Warung","category":"Dining & Food","amount":4000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp296193394","date":"2026-02-04","description":"Syifa","category":"Ritual Penggandaan Rezeki","amount":3000000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp729423394","date":"2026-02-05","description":"S (3 & 5 Februari)","category":"Other Income","amount":1500000,"type":"income","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp467363394","date":"2026-02-05","description":"Kopi Warung","category":"Dining & Food","amount":4000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp647623394","date":"2026-02-05","description":"Desk Shelf","category":"Shopping","amount":113200,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp455123394","date":"2026-02-05","description":"Terea Gojek","category":"Dining & Food","amount":100000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp579023394","date":"2026-02-05","description":"Media Tanam","category":"Hobbies","amount":25000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp607133394","date":"2026-02-06","description":"Kopi Warung","category":"Dining & Food","amount":4000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp691633394","date":"2026-02-06","description":"Kokuo","category":"Self Care","amount":250000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp363253394","date":"2026-02-06","description":"Nasi Goreng","category":"Groceries","amount":30000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp608853394","date":"2026-02-06","description":"Bensin Cibubur","category":"Transport & Fuel","amount":331238,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp430373394","date":"2026-02-07","description":"Anthurium Warocq","category":"Hobbies","amount":350000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp116963394","date":"2026-02-07","description":"Topup Lalamove","category":"Hobbies","amount":50000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp413083394","date":"2026-02-08","description":"Pentol","category":"Groceries","amount":100000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp564183394","date":"2026-02-08","description":"Mie Ayam Umi","category":"Groceries","amount":36000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp434793394","date":"2026-02-08","description":"Alfamart","category":"Dining & Food","amount":110000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp163733394","date":"2026-02-08","description":"Baju Ayah","category":"Ritual Penggandaan Rezeki","amount":700000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp805313394","date":"2026-02-08","description":"WiFi Indihome","category":"Mobile Data & Internet","amount":366637,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp471753394","date":"2026-02-09","description":"E Tol","category":"Transport & Fuel","amount":360000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp325973394","date":"2026-02-09","description":"Tanaman","category":"Hobbies","amount":96075,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp220203395","date":"2026-02-09","description":"Remot AC","category":"Shopping","amount":30700,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp352583395","date":"2026-02-09","description":"Arisan","category":"Savings","amount":200000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp348913394","date":"2026-02-10","description":"M","category":"Other Income","amount":500000,"type":"income","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp187903394","date":"2026-02-10","description":"Kopi Warung","category":"Dining & Food","amount":4000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp985023395","date":"2026-02-10","description":"Token Listrik","category":"Mobile Data & Internet","amount":201100,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp390493394","date":"2026-02-11","description":"S (10&11 Februari)","category":"Other Income","amount":1200000,"type":"income","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp828273395","date":"2026-02-11","description":"Turus Lumut","category":"Hobbies","amount":34400,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp122833395","date":"2026-02-12","description":"Kopi Warung","category":"Dining & Food","amount":12000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp557003395","date":"2026-02-12","description":"FC 26","category":"Hobbies","amount":95900,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp780763395","date":"2026-02-12","description":"Terea Gopay Topup","category":"Dining & Food","amount":89000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp205983395","date":"2026-02-13","description":"Kopi Nako Gopay Topup","category":"Dining & Food","amount":116000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp147283395","date":"2026-02-13","description":"Philodendron Burle Marx","category":"Hobbies","amount":100000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp215373395","date":"2026-02-14","description":"Jasa Biwir","category":"Ritual Penggandaan Rezeki","amount":170666,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp788643395","date":"2026-02-14","description":"Sekam Bakar","category":"Hobbies","amount":23200,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp371323395","date":"2026-02-14","description":"Pupuk Kompos","category":"Hobbies","amount":66800,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp428923395","date":"2026-02-14","description":"Alat berkebun","category":"Hobbies","amount":55650,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp606453395","date":"2026-02-14","description":"Kaede Coffee","category":"Dining & Food","amount":60500,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp260023395","date":"2026-02-16","description":"Terea","category":"Dining & Food","amount":70000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp469683395","date":"2026-02-16","description":"Biwir","category":"Ritual Penggandaan Rezeki","amount":36000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp727733395","date":"2026-02-16","description":"Kawan Baru","category":"Dining & Food","amount":74700,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp962793395","date":"2026-02-16","description":"Bensin","category":"Transport & Fuel","amount":400000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp898393395","date":"2026-02-16","description":"E Tol","category":"Transport & Fuel","amount":360000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp550273394","date":"2026-02-18","description":"Munggahan Kantor","category":"Other Income","amount":300000,"type":"income","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp744673395","date":"2026-02-18","description":"Makan Warung","category":"Groceries","amount":20000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp271923395","date":"2026-02-18","description":"Shampoo Kelaya","category":"Self Care","amount":73350,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp762683395","date":"2026-02-18","description":"Terea + Cheat PB","category":"Dining & Food","amount":121000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp505313394","date":"2026-02-19","description":"M","category":"Other Income","amount":500000,"type":"income","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp883433395","date":"2026-02-20","description":"Oli & Etc. motor","category":"Transport & Fuel","amount":182195,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp904633395","date":"2026-02-20","description":"Kurma","category":"Groceries","amount":60800,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp578553395","date":"2026-02-21","description":"Gopay Topup","category":"Shopping","amount":122000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp977703395","date":"2026-02-21","description":"Pulsa XL","category":"Mobile Data & Internet","amount":100000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp976903395","date":"2026-02-21","description":"Kopi Ud Djaya","category":"Dining & Food","amount":60500,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp925983395","date":"2026-02-21","description":"Sil Tip Shockbreaker","category":"Shopping","amount":100000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp642113395","date":"2026-02-21","description":"Bensin","category":"Transport & Fuel","amount":200000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp188663395","date":"2026-02-22","description":"Belajar Clip","category":"Subscriptions","amount":103400,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp263983395","date":"2026-02-22","description":"E Tol","category":"Transport & Fuel","amount":300000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp824173394","date":"2026-02-24","description":"M","category":"Other Income","amount":500000,"type":"income","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp668523395","date":"2026-02-24","description":"Bensin","category":"Transport & Fuel","amount":347000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp184523395","date":"2026-02-24","description":"Martabak","category":"Groceries","amount":45000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp764723395","date":"2026-02-24","description":"DP Bukber 7 Maret","category":"Groceries","amount":240000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp108483395","date":"2026-02-25","description":"Shopee Pay (Ubi)","category":"Shopping","amount":31260,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp731093395","date":"2026-02-26","description":"E Tol","category":"Transport & Fuel","amount":150000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp985193394","date":"2026-02-27","description":"S","category":"Other Income","amount":100000,"type":"income","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp280013395","date":"2026-02-27","description":"Gopay Topup (Blenger)","category":"Groceries","amount":151000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp899283394","date":"2026-02-28","description":"M","category":"Other Income","amount":257000,"type":"income","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp358783395","date":"2026-02-28","description":"PT PT Padel","category":"Hobbies","amount":100000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp292683395","date":"2026-02-28","description":"Admin, parkir & etc","category":"Shopping","amount":150000,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp478093395","date":"2026-02-28","description":"Pindah Budget Ke Maret","category":"Savings","amount":1769442,"type":"expense","wallet":"Cash","notes":"Imported from FEBRUARY"},{"id":"imp451833520","date":"2026-03-01","description":"Sisa Februari","category":"Other Income","amount":1769442,"type":"income","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp557733520","date":"2026-03-01","description":"Token Listrik","category":"Mobile Data & Internet","amount":306700,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp967323520","date":"2026-03-02","description":"Month Salary","category":"Monthly Salary","amount":4500000,"type":"income","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp334783520","date":"2026-03-02","description":"Verdant & Uncle Jo","category":"Dining & Food","amount":105000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp408203520","date":"2026-03-02","description":"S","category":"Other Income","amount":100000,"type":"income","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp630373520","date":"2026-03-03","description":"Bensin BP","category":"Transport & Fuel","amount":330000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp109213520","date":"2026-03-03","description":"Terea & Greek Youg","category":"Shopping","amount":58900,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp732963520","date":"2026-03-04","description":"Arisan","category":"Savings","amount":200000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp946493520","date":"2026-03-05","description":"E Money","category":"Transport & Fuel","amount":300000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp192923520","date":"2026-03-05","description":"Microfiber dan Pembersih","category":"Shopping","amount":74214,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp520943520","date":"2026-03-06","description":"M","category":"Other Income","amount":350000,"type":"income","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp911523520","date":"2026-03-06","description":"Syifa","category":"Ritual Penggandaan Rezeki","amount":3000000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp673873520","date":"2026-03-06","description":"Warbew","category":"Dining & Food","amount":17000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp867013520","date":"2026-03-06","description":"Hvbitat","category":"Dining & Food","amount":235900,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp184993520","date":"2026-03-06","description":"MCD","category":"Groceries","amount":102500,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp718753520","date":"2026-03-07","description":"FC 26","category":"Shopping","amount":61900,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp860003520","date":"2026-03-07","description":"Bukber Cipete","category":"Groceries","amount":56000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp329113520","date":"2026-03-07","description":"E Tol (gantiin syifa)","category":"Transport & Fuel","amount":300000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp933793520","date":"2026-03-07","description":"Beras","category":"Groceries","amount":58000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp151733520","date":"2026-03-07","description":"Terea","category":"Dining & Food","amount":70000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp208333520","date":"2026-03-08","description":"M","category":"Other Income","amount":300000,"type":"income","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp521413520","date":"2026-03-10","description":"Apple Music (Topup Dana)","category":"Subscriptions","amount":60000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp355063520","date":"2026-03-10","description":"Konidin","category":"Self Care","amount":83229,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp651453520","date":"2026-03-11","description":"Sarung tangan cuci mobil","category":"Shopping","amount":24716,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp276143520","date":"2026-03-11","description":"Baju Syifa","category":"Shopping","amount":73654,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp264073520","date":"2026-03-11","description":"Taneman","category":"Hobbies","amount":127500,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp219843520","date":"2026-03-11","description":"Bensin","category":"Transport & Fuel","amount":342000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp910753520","date":"2026-03-12","description":"Biwir","category":"Ritual Penggandaan Rezeki","amount":72000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp953293520","date":"2026-03-13","description":"THR","category":"Other Income","amount":4400000,"type":"income","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp510393520","date":"2026-03-13","description":"Warbew","category":"Dining & Food","amount":26000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp680643520","date":"2026-03-13","description":"Terea","category":"Dining & Food","amount":70000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp452263520","date":"2026-03-13","description":"Kopi Klan","category":"Dining & Food","amount":135300,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp479063520","date":"2026-03-13","description":"Nasi Pada Trio","category":"Groceries","amount":54000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp496873520","date":"2026-03-13","description":"E Tol (gantiin syifa)","category":"Transport & Fuel","amount":100000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp930343520","date":"2026-03-13","description":"Kopi Klan","category":"Dining & Food","amount":135300,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp840033520","date":"2026-03-14","description":"Halaman Bhumi","category":"Dining & Food","amount":68000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp856193520","date":"2026-03-15","description":"Token Listrik","category":"Mobile Data & Internet","amount":104100,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp674393520","date":"2026-03-15","description":"TUKU","category":"Dining & Food","amount":300000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp555403520","date":"2026-03-15","description":"Bensin PCX","category":"Transport & Fuel","amount":92000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp150473520","date":"2026-03-16","description":"M","category":"Other Income","amount":300000,"type":"income","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp278153520","date":"2026-03-16","description":"THR Kantor","category":"Other Income","amount":1500000,"type":"income","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp698673520","date":"2026-03-16","description":"13 & 16 E Tol","category":"Transport & Fuel","amount":400000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp144073520","date":"2026-03-16","description":"THR Syifa","category":"Ritual Penggandaan Rezeki","amount":1500000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp179803520","date":"2026-03-18","description":"IPL & Iuran","category":"Subscriptions","amount":560000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp628423520","date":"2026-03-18","description":"Token Listrik","category":"Mobile Data & Internet","amount":104100,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp324793520","date":"2026-03-18","description":"Parcel Satpam","category":"Ritual Penggandaan Rezeki","amount":252900,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp418653520","date":"2026-03-19","description":"Apple Music (Dana Topup)","category":"Subscriptions","amount":100000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp214593520","date":"2026-03-19","description":"Terea","category":"Dining & Food","amount":70000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp813123520","date":"2026-03-19","description":"Maranggi Sari Asih","category":"Groceries","amount":78000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp686293520","date":"2026-03-20","description":"Ade Azhar","category":"Ritual Penggandaan Rezeki","amount":600000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp268523520","date":"2026-03-20","description":"Bensin","category":"Transport & Fuel","amount":200000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp228513520","date":"2026-03-20","description":"Gorengan","category":"Groceries","amount":50000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp118773520","date":"2026-03-21","description":"THR Ayah Ibu","category":"Other Income","amount":1000000,"type":"income","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp826493520","date":"2026-03-21","description":"THR","category":"Other Income","amount":425000,"type":"income","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp333393520","date":"2026-03-21","description":"Pulsa XL","category":"Mobile Data & Internet","amount":51099,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp320863520","date":"2026-03-21","description":"Pulsa XL","category":"Mobile Data & Internet","amount":51099,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp588653520","date":"2026-03-21","description":"THR ade","category":"Ritual Penggandaan Rezeki","amount":200000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp387473520","date":"2026-03-23","description":"Kawan Baru","category":"Dining & Food","amount":80400,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp150703520","date":"2026-03-23","description":"Parcel Susulan","category":"Shopping","amount":97100,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp659893520","date":"2026-03-23","description":"Parcel Susulan","category":"Shopping","amount":38000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp977933520","date":"2026-03-23","description":"Halaman Bhumi","category":"Dining & Food","amount":30000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp824933520","date":"2026-03-23","description":"Plutos Coffee","category":"Dining & Food","amount":152900,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp557833520","date":"2026-03-23","description":"Topup Steam Wallet","category":"Hobbies","amount":120659,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp266083520","date":"2026-03-23","description":"Terea","category":"Dining & Food","amount":72000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp176313520","date":"2026-03-24","description":"Sedari Coffee","category":"Dining & Food","amount":55000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp763913520","date":"2026-03-25","description":"Pompa Air Kolam","category":"Shopping","amount":252555,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp231263520","date":"2026-03-25","description":"Steam Wallet","category":"Hobbies","amount":58523,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp393403520","date":"2026-03-25","description":"Makan (Gopay Topup)","category":"Groceries","amount":36000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp434963520","date":"2026-03-25","description":"Kopi Klan","category":"Dining & Food","amount":52800,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp625953520","date":"2026-03-25","description":"Bensin","category":"Transport & Fuel","amount":340000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp851863520","date":"2026-03-26","description":"Biwir","category":"Ritual Penggandaan Rezeki","amount":150000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp342043520","date":"2026-03-27","description":"Gocar (Gopay Topup)","category":"Transport & Fuel","amount":83000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp454683520","date":"2026-03-27","description":"Servis Mobil","category":"Transport & Fuel","amount":1735925,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp118573520","date":"2026-03-27","description":"Terea","category":"Transport & Fuel","amount":70000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp229763520","date":"2026-03-27","description":"Gyu Kaku","category":"Groceries","amount":710424,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp657423520","date":"2026-03-28","description":"E Tol","category":"Transport & Fuel","amount":350000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp401983520","date":"2026-03-29","description":"Jajan Pasar Bersih Sentul","category":"Dining & Food","amount":40000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp590183520","date":"2026-03-29","description":"Token Listrik","category":"Mobile Data & Internet","amount":104100,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp620333520","date":"2026-03-30","description":"Point Coffee","category":"Dining & Food","amount":25000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp518143520","date":"2026-03-31","description":"Parkir, Admin & etc","category":"Others","amount":100000,"type":"expense","wallet":"Cash","notes":"Imported from MARCH"},{"id":"imp264683636","date":"2026-04-01","description":"M","category":"Other Income","amount":300000,"type":"income","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp113113636","date":"2026-04-01","description":"Roko Terea","category":"Dining & Food","amount":60000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp553683636","date":"2026-04-01","description":"Month Salary","category":"Monthly Salary","amount":4400000,"type":"income","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp573943636","date":"2026-04-01","description":"Kopi, Rokok","category":"Dining & Food","amount":70000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp539993636","date":"2026-04-01","description":"Soto Ayam Pomad","category":"Groceries","amount":29000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp349333636","date":"2026-04-02","description":"Racun Rumput","category":"Shopping","amount":16630,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp829283636","date":"2026-04-02","description":"Token Listrik","category":"Mobile Data & Internet","amount":501750,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp910563636","date":"2026-04-03","description":"Jajan Pagi Tukang","category":"Dining & Food","amount":47500,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp612123636","date":"2026-04-03","description":"Terea","category":"Dining & Food","amount":72000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp359853636","date":"2026-04-03","description":"Phone Holder","category":"Shopping","amount":96456,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp848663636","date":"2026-04-03","description":"Car Freshner","category":"Shopping","amount":34958,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp436883636","date":"2026-04-04","description":"EM4","category":"Shopping","amount":24500,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp886023636","date":"2026-04-04","description":"Syifa","category":"Ritual Penggandaan Rezeki","amount":3000000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp268903636","date":"2026-04-04","description":"Twenty Two Coffee Bellanova","category":"Dining & Food","amount":194700,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp541593636","date":"2026-04-04","description":"Penutup Kca Mobil","category":"Shopping","amount":34660,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp728163636","date":"2026-04-04","description":"Intis","category":"Dining & Food","amount":121000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp984953636","date":"2026-04-06","description":"Arisan","category":"Savings","amount":200000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp511083636","date":"2026-04-06","description":"Brezza","category":"Dining & Food","amount":35000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp491993636","date":"2026-04-06","description":"E Tol","category":"Transport & Fuel","amount":300000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp297893636","date":"2026-04-07","description":"M","category":"Other Income","amount":548200,"type":"income","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp656733636","date":"2026-04-07","description":"Terea+Maizena","category":"Dining & Food","amount":37500,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp266123636","date":"2026-04-08","description":"Kopi Indomie Warung","category":"Dining & Food","amount":15000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp859973636","date":"2026-04-08","description":"Indihome","category":"Mobile Data & Internet","amount":366637,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp165183636","date":"2026-04-08","description":"IPL","category":"Subscriptions","amount":180000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp458773636","date":"2026-04-09","description":"Terea","category":"Dining & Food","amount":72000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp251123636","date":"2026-04-09","description":"Glaco Coating Glass","category":"Shopping","amount":251730,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp195653636","date":"2026-04-10","description":"Baju","category":"Shopping","amount":145195,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp291673636","date":"2026-04-11","description":"Kouta XL","category":"Mobile Data & Internet","amount":80000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp436803636","date":"2026-04-11","description":"Nasi Goreng","category":"Groceries","amount":30000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp737923636","date":"2026-04-11","description":"E Tol","category":"Transport & Fuel","amount":300000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp558113636","date":"2026-04-11","description":"Blackurant","category":"Dining & Food","amount":66000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp736783636","date":"2026-04-12","description":"Timun Mas Coffee Lab","category":"Dining & Food","amount":143000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp995343636","date":"2026-04-12","description":"Makanan Aceh Sentul","category":"Groceries","amount":52000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp228603636","date":"2026-04-12","description":"Terea","category":"Dining & Food","amount":108000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp617053636","date":"2026-04-12","description":"Tukang","category":"Ritual Penggandaan Rezeki","amount":50000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp343253636","date":"2026-04-13","description":"M","category":"Other Income","amount":300000,"type":"income","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp631053636","date":"2026-04-13","description":"Topup Shopee Pay","category":"Shopping","amount":50000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp934633636","date":"2026-04-13","description":"Microfiber Lap + Dryer","category":"Shopping","amount":101100,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp902543636","date":"2026-04-13","description":"Kopi Warung","category":"Dining & Food","amount":5000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp663493636","date":"2026-04-13","description":"Cukur","category":"Self Care","amount":55000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp884843636","date":"2026-04-15","description":"Gopay Topup (Terea)","category":"Dining & Food","amount":90000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp944823636","date":"2026-04-15","description":"Token Listrik","category":"Mobile Data & Internet","amount":500000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp267433636","date":"2026-04-15","description":"Piscok + Es Jeruk 16 Apr","category":"Dining & Food","amount":15000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp156963636","date":"2026-04-15","description":"E Tol","category":"Transport & Fuel","amount":300000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp733863636","date":"2026-04-16","description":"IEM Kinera Wyvern","category":"Shopping","amount":273516,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp465323636","date":"2026-04-17","description":"Gopay Topup (Terea)","category":"Dining & Food","amount":55000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp574483636","date":"2026-04-19","description":"Gopay Topup (ES Teh)","category":"Dining & Food","amount":48000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp285473636","date":"2026-04-20","description":"M","category":"Other Income","amount":300000,"type":"income","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp656593636","date":"2026-04-20","description":"Magnesium","category":"Self Care","amount":73778,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp245743636","date":"2026-04-20","description":"Apple Cloud Musid (Dana)","category":"Subscriptions","amount":110000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp590313636","date":"2026-04-20","description":"Claude (Dana Topup)","category":"Subscriptions","amount":350000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp664203636","date":"2026-04-22","description":"Baju Olahraga Syifa","category":"Ritual Penggandaan Rezeki","amount":219720,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp572583636","date":"2026-04-23","description":"biwir","category":"Ritual Penggandaan Rezeki","amount":100000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp322093636","date":"2026-04-24","description":"Rahang Tuna","category":"Groceries","amount":90000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp122793636","date":"2026-04-25","description":"E Money (21&25 April)","category":"Transport & Fuel","amount":550000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp777443636","date":"2026-04-25","description":"Bonex","category":"Groceries","amount":53000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp746213636","date":"2026-04-26","description":"Warisan Kopi","category":"Dining & Food","amount":60000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp233813636","date":"2026-04-26","description":"PLN Mobile","category":"Mobile Data & Internet","amount":501750,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp289903636","date":"2026-04-26","description":"Naturehike Dry Bag","category":"Shopping","amount":182851,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp348803636","date":"2026-04-26","description":"Coca Cola","category":"Dining & Food","amount":15000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp646073636","date":"2026-04-27","description":"M","category":"Other Income","amount":300000,"type":"income","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp663143636","date":"2026-04-28","description":"Beras","category":"Groceries","amount":74724,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp854443636","date":"2026-04-29","description":"S","category":"Other Income","amount":150000,"type":"income","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp119603636","date":"2026-04-29","description":"1800 + Black Label","category":"Dining & Food","amount":1100000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp768443636","date":"2026-04-29","description":"Topron (ShopeePay Topup)","category":"Shopping","amount":36000,"type":"expense","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp144873636","date":"2026-04-30","description":"S","category":"Other Income","amount":300000,"type":"income","wallet":"Cash","notes":"Imported from APRIL"},{"id":"imp656433760","date":"2026-05-02","description":"ETol","category":"Transport & Fuel","amount":300000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp745493760","date":"2026-05-02","description":"Tyo + Tomyum","category":"Ritual Penggandaan Rezeki","amount":300000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp978303760","date":"2026-05-02","description":"Ayam Taliwang","category":"Groceries","amount":103000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp634993760","date":"2026-05-03","description":"COD MW Remastered","category":"Hobbies","amount":19200,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp309573760","date":"2026-05-05","description":"Month Salary","category":"Monthly Salary","amount":4500000,"type":"income","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp355773760","date":"2026-05-05","description":"Pull Up Bar (Shopeepay)","category":"Shopping","amount":120000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp266553760","date":"2026-05-05","description":"Syifa","category":"Ritual Penggandaan Rezeki","amount":3000000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp421093760","date":"2026-05-05","description":"Kouta XL","category":"Mobile Data & Internet","amount":80000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp948233760","date":"2026-05-05","description":"Nasi Goreng (Gopay Topup)","category":"Groceries","amount":66000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp191283760","date":"2026-05-05","description":"Admin Jenius","category":"Subscriptions","amount":20000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp467833760","date":"2026-05-06","description":"Extra Income","category":"Other Income","amount":300000,"type":"income","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp494733760","date":"2026-05-07","description":"Token Listrik","category":"Mobile Data & Internet","amount":501750,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp246733760","date":"2026-05-07","description":"IPL","category":"Subscriptions","amount":180000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp823523760","date":"2026-05-07","description":"Makan","category":"Groceries","amount":20000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp113523760","date":"2026-05-08","description":"Aris","category":"Other Income","amount":2800000,"type":"income","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp746333760","date":"2026-05-08","description":"SPPD","category":"Other Income","amount":100000,"type":"income","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp265583760","date":"2026-05-08","description":"Cuci Mobil","category":"Transport & Fuel","amount":40000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp572943760","date":"2026-05-09","description":"Insole","category":"Shopping","amount":128949,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp913243760","date":"2026-05-10","description":"Ud Djaya","category":"Dining & Food","amount":51700,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp607443760","date":"2026-05-10","description":"Pisang Pempek, Tekwan","category":"Groceries","amount":85000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp963443760","date":"2026-05-10","description":"Cukur","category":"Self Care","amount":40000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp448923760","date":"2026-05-10","description":"ETol","category":"Transport & Fuel","amount":200000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp966113760","date":"2026-05-12","description":"Extra Income","category":"Other Income","amount":300000,"type":"income","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp372113760","date":"2026-05-13","description":"Makan Warung","category":"Dining & Food","amount":19000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp368753760","date":"2026-05-13","description":"Lightroom","category":"Subscriptions","amount":30000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp702453760","date":"2026-05-13","description":"Beard Papa","category":"Groceries","amount":48000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp977383760","date":"2026-05-16","description":"Ikan Parung Iko","category":"Hobbies","amount":480000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp141153760","date":"2026-05-17","description":"Japanese Food","category":"Groceries","amount":94000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp439253760","date":"2026-05-17","description":"ETol","category":"Transport & Fuel","amount":750000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp902273760","date":"2026-05-18","description":"Kouta XL","category":"Mobile Data & Internet","amount":80000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp654623760","date":"2026-05-18","description":"Internet","category":"Mobile Data & Internet","amount":367800,"type":"expense","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp464223760","date":"2026-05-19","description":"Extra Income","category":"Other Income","amount":300000,"type":"income","wallet":"Cash","notes":"Imported from MAY"},{"id":"imp744213760","date":"2026-05-31","description":"Pengeluaran lainnya","category":"Others","amount":25000,"type":"expense","wallet":"Cash","notes":"Imported from MAY"}];
...
function UserApp({user,onLogout}){
  ...
  useEffect(()=>{
    loadData(user.id).then(d=>{
      const applied=applyRecurring(d);
      // Auto-embed Putu's historical data ONCE — skip if already imported before
      const isPutu=user?.name?.toUpperCase()==="PUTU"||user?.id==="u1";
      if(isPutu&&!applied.importDone){
        const merged={...applied,
          transactions:[...applied.transactions,...PUTU_IMPORT_DATA].sort((a,b)=>a.date.localeCompare(b.date)),
          importDone:true
        };
        saveData(merged,user.id);
        setData(merged);
  ...
  // darkMode saved inline when toggled
