...
function Transactions({txs,data,updateData}){
  ...
  const startEdit=(t,el)=>{
    ...
    // scroll to element position for modal anchor
    if(el){const rect=el.getBoundingClientRect();setEditAnchorY(rect.top+window.scrollY);}
  };
  ...
  return(
    <div style={{padding:"16px 14px 100px"}}>
      {/* Edit Modal — inline, anchored to tapped item */}
      {editTx&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:200,overflowY:"auto",display:"flex",alignItems:"flex-start",paddingTop:Math.max(editAnchorY-60,60)}} onClick={()=>setEditTx(null)}>
          <div style={{...card,borderRadius:24,padding:"20px 20px 28px",width:"calc(100% - 32px)",maxWidth:448,margin:"0 auto 40px"}} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
              ...
              {editTx.type!=="transfer"&&(
                <div><div style={{fontSize:11,color:C.muted,fontWeight:700,letterSpacing:1,marginBottom:5}}>CATEGORY</div>
              ...
              <div><div style={{fontSize:11,color:C.muted,fontWeight:700,letterSpacing:1,marginBottom:5}}>WALLET</div>
              ...
              <div><div style={{fontSize:11,color:C.muted,fontWeight:700,letterSpacing:1,marginBottom:5}}>AMOUNT (IDR)</div><input type="text" inputMode="numeric" value={editForm.amount} onChange={e=>setEditForm(f=>({...f,amount:fmtE(e.target.value)}))} style={{...inpE,fontFamily:MONO,fontWeight:700,color:editTx.type==="income"?C.mintD:C.coral}}/></div>
              <div><div style={{fontSize:11,color:C.muted,fontWeight:700,letterSpacing:1,marginBottom:5}}>NOTES (optional)</div><input type="text" placeholder="Optional note..." value={editForm.notes||""} onChange={e=>setEditForm(f=>({...f,notes:e.target.value}))} style={inpE}/></div>
            </div>
          ...
          <div style={{textAlign:"right",flexShrink:0}}>
            ...
            <div style={{display:"flex",gap:5,marginTop:5,justifyContent:"flex-end"}}>
              {confirmId===t.id
                ...
                :<><button onClick={e=>{startEdit(t,e.currentTarget.closest('[data-txcard]'));}} style={{background:`${C.blue}10`,border:"none",borderRadius:6,width:26,height:26,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0}}>
