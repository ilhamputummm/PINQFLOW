...
function ImportPage({data,updateData,onBack}){
  const C=useTheme();
  const card=mkCard(C),cardSm=mkCardSm(C);
  const [status,setStatus]=useState("idle"); // idle | reading | preview | importing | done | undone | error
  ...
  useEffect(()=>{
    ...
    s.onerror=()=>setErrorMsg("Failed to load Excel reader library");
    document.head.appendChild(s);
  },[]);

  const INCOME_MAP={
    "month salary":"Monthly Salary","extra income":"Other Income","sppd":"Other Income",
    "aris":"Other Income","thr":"Other Income","bonus":"Bonus","freelance":"Freelance",
  };
  const EXPENSE_MAP={
    "lifestyle - hangout, jajan & entertaintment":"Dining & Food",
    "lifestyle - hangout":"Dining & Food",
    "lifestyle - monthly subscription":"Subscriptions",
    "personal need's - food":"Groceries","personal need's - self care":"Self Care",
    "personal need's self care":"Self Care","personal need's hobby":"Hobbies",
    "personal need's - hobby":"Hobbies","personal need's h":"Groceries",
    "personal need's hobby":"Hobbies",
    "transport/gas/tol/car motor":"Transport & Fuel","transport/gas/tol/":"Transport & Fuel",
    "pulsa & internet data":"Mobile Data & Internet","ritual penggandaan rezeki":"Ritual Penggandaan Rezeki",
    "monthly saving's":"Savings","shopping":"Shopping","invest reksadana / saham":"Investment",
    "utilities":"Utilities","tobacco":"Tobacco",
  };

  const mapIncCat=(cat)=>{
    if(!cat)return"Other Income";
    const c=cat.trim().toLowerCase();
    for(const[k,v] of Object.entries(INCOME_MAP)){if(c.includes(k))return v;}
    return"Other Income";
  };
  const mapExpCat=(cat)=>{
    if(!cat)return"Others";
    const c=cat.trim().toLowerCase();
    for(const[k,v] of Object.entries(EXPENSE_MAP)){if(c.includes(k))return v;}
    return"Others";
  };
  ...
  const excelDateToString=(v)=>{
    ...
    if(v instanceof Date||typeof v==="object"&&v?.getFullYear){
      const y=v.getFullYear(),m=v.getMonth()+1,dd=v.getDate();
      if(y<2020||y>2030)return null;
      return `${y}-${String(m).padStart(2,"0")}-${String(dd).padStart(2,"0")}`;
    }
  ...
  const parseExcel=(file)=>{
    ...
    reader.onload=(e)=>{
      try{
        ...
        const allTxs=[];
        const sheetNames=["JANUARY","FEBRUARY","MARCH","APRIL","MAY","JUNE","JULY","AUGUST","SEPTEMBER","OCTOBER","NOVEMBER","DECEMBER"];
        const found=wb.SheetNames.filter(n=>sheetNames.includes(n.toUpperCase()));
        if(found.length===0)throw new Error("No monthly sheets found. Make sure your Excel has sheets named JANUARY, FEBRUARY, etc.");

        found.forEach(sheetName=>{
          ...
          rows.slice(4).forEach(row=>{
            ...
            // Income: cols 1(No),2(Date),3(Desc),4(Cat),5(Amount)
            const iNo=row[1],iDate=row[2],iDesc=row[3],iCat=row[4],iAmt=row[5];
            // Expense: cols 7(No),8(Date),9(Desc),10(Cat),11(Amount)
            const eNo=row[7],eDate=row[8],eDesc=row[9],eCat=row[10],eAmt=row[11];

            // Process income
            if(iNo&&iDate&&iDesc&&iAmt){
              const amt=typeof iAmt==="number"?iAmt:parseInt(String(iAmt).replace(/[^0-9]/g,""))||0;
              const date=excelDateToString(iDate);
              const desc=String(iDesc).trim();
              if(amt>0&&date&&!desc.toLowerCase().includes("sisa")){
                allTxs.push({id:"imp"+Math.random().toString(36).slice(2)+Date.now().toString(36).slice(-4),date,description:desc,category:mapIncCat(iCat),amount:amt,type:"income",wallet:"Cash",notes:`Imported from ${sheetName}`});
              }
            }
            // Process expense
            if(eNo&&eDate&&eDesc&&eAmt){
              const amt=typeof eAmt==="number"?eAmt:parseInt(String(eAmt).replace(/[^0-9]/g,""))||0;
              const date=excelDateToString(eDate);
              const desc=String(eDesc).trim();
              if(amt>0&&date){
                allTxs.push({id:"imp"+Math.random().toString(36).slice(2)+Date.now().toString(36).slice(-4),date,description:desc,category:mapExpCat(eCat),amount:amt,type:"expense",wallet:"Cash",notes:`Imported from ${sheetName}`});
              }
            }
          });
        });
        ...
        const existingKeys=new Set(data.transactions.map(t=>t.date+"_"+t.description+"_"+t.amount));
        const fresh=allTxs.filter(t=>!existingKeys.has(t.date+"_"+t.description+"_"+t.amount));
        setPreviewTxs(fresh);
        setStatus("preview");
      }catch(err){
  ...
  const handleFile=(e)=>{
    ...
    if(!file)return;
    if(!file.name.match(/\.xlsx?$/i)){setErrorMsg("Please upload an .xlsx file");setStatus("error");return;}
    parseExcel(file);
  };
  ...
  const handleUndo=async()=>{
    ...
    setStatus("undone");setImportedIds([]);setPreviewTxs([]);
  };

  const reset=()=>{setStatus("idle");setPreviewTxs([]);setErrorMsg("");setFileName("");if(fileRef.current)fileRef.current.value="";};
  ...
  return(
    <div style={{padding:"0 0 80px"}}>
      {/* SheetJS loaded via useEffect below */}
      <BackBtn onBack={onBack}/>
      <div style={{fontSize:18,fontWeight:900,color:C.navy,marginBottom:4,letterSpacing:-0.5}}>📥 Import Data</div>
      <div style={{fontSize:12,color:C.muted,marginBottom:20}}>Upload your Excel spreadsheet (.xlsx)</div>

      {/* IDLE — upload area */}
      {status==="idle"&&(
        <>
          <input ref={fileRef} type="file" accept=".xlsx,.xls" onChange={handleFile} style={{display:"none"}} id="xlsx-upload"/>
          <label htmlFor="xlsx-upload" style={{display:"block",border:`2px dashed ${C.mint}60`,borderRadius:20,padding:"32px 20px",textAlign:"center",cursor:"pointer",background:isDark?"rgba(52,211,153,0.05)":"rgba(52,211,153,0.03)",marginBottom:16}}>
            <div style={{fontSize:40,marginBottom:10}}>📂</div>
            <div style={{fontSize:14,fontWeight:700,color:C.navy,marginBottom:4}}>Tap to choose Excel file</div>
            <div style={{fontSize:11,color:C.muted}}>Supports .xlsx with monthly sheets</div>
            <div style={{marginTop:14,display:"inline-block",background:xlsxReady?`linear-gradient(135deg,${C.mint},${C.mintD})`:`${C.border}`,borderRadius:12,padding:"10px 24px",color:xlsxReady?"#fff":C.muted,fontSize:13,fontWeight:700}}>{xlsxReady?"Choose File":"Loading..."}</div>
          </label>
          <div style={{...cardSm,padding:"12px 16px",background:`${C.blue}10`,border:`1px solid ${C.blue}30`}}>
            <div style={{fontSize:11,color:C.blue,fontWeight:700,marginBottom:6}}>ℹ️ Format yang didukung</div>
            <div style={{fontSize:11,color:C.muted,lineHeight:1.8}}>
              • Sheet per bulan (JANUARY, FEBRUARY, dst)<br/>
              • Kolom: No, Date, Description, Category, Amount<br/>
              • Income & Expense side by side<br/>
              • Kategori otomatis di-mapping ke PinqFlow
            </div>
          </div>
        </>
      )}
      ...
      {status==="reading"&&(
        <div style={{textAlign:"center",padding:"48px 0"}}>
          ...
          <div style={{fontSize:14,fontWeight:700,color:C.navy,marginBottom:4}}>Reading {fileName}...</div>
          <div style={{fontSize:12,color:C.muted}}>Parsing all sheets</div>
        </div>
      )}
      ...
      {status==="error"&&(
        <>
          <div style={{...card,padding:"20px",marginBottom:16,textAlign:"center",border:`1.5px solid ${C.coral}40`}}>
            <div style={{fontSize:36,marginBottom:10}}>❌</div>
            <div style={{fontSize:14,fontWeight:700,color:C.coral,marginBottom:6}}>Failed to read file</div>
            <div style={{fontSize:12,color:C.muted}}>{errorMsg}</div>
          </div>
          <button onClick={reset} style={{width:"100%",background:C.border,border:"none",borderRadius:14,padding:"14px 0",color:C.muted,fontSize:14,fontWeight:700,cursor:"pointer"}}>Try Again</button>
        </>
      )}
      ...
      {status==="preview"&&(
        <>
          <div style={{...cardSm,padding:"12px 16px",marginBottom:12,background:`${C.mint}10`,border:`1px solid ${C.mint}30`}}>
            <div style={{fontSize:12,fontWeight:700,color:C.mintD,marginBottom:2}}>✅ File read: {fileName}</div>
            <div style={{fontSize:11,color:C.muted}}>{previewTxs.length} new transactions found</div>
          </div>
          <div style={{...card,padding:"16px 18px",marginBottom:14}}>
            ...
            {Object.entries(byMonth).sort().map(([m,d])=>{
              ...
              return(
                <div key={m} style={{marginBottom:12}}>
                  <div style={{fontSize:12,fontWeight:700,color:C.navy,marginBottom:6}}>{MN[+mo]} {y} <span style={{fontSize:10,color:C.muted,fontWeight:500}}>({d.count} items)</span></div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                    <div style={{background:`${C.mintD}12`,borderRadius:10,padding:"8px 10px"}}>
                      ...
                      <div style={{fontFamily:MONO,fontSize:12,fontWeight:700,color:C.mintD}}>{rpK(d.inc)}</div>
                    </div>
                    <div style={{background:`${C.coral}12`,borderRadius:10,padding:"8px 10px"}}>
                      ...
                      <div style={{fontFamily:MONO,fontSize:12,fontWeight:700,color:C.coral}}>{rpK(d.exp)}</div>
                    </div>
                  </div>
                </div>
              );
            ...
            {previewTxs.length===0&&(
              <div style={{textAlign:"center",padding:"16px 0",color:C.muted,fontSize:13}}>All transactions already imported!</div>
            )}
            <div style={{borderTop:`1px solid ${C.border}`,paddingTop:10,marginTop:4}}>
              <div style={{fontSize:11,color:C.muted}}>You can undo this anytime after import</div>
            </div>
          </div>
          <div style={{display:"flex",gap:10}}>
            <button onClick={reset} style={{flex:1,background:C.border,border:"none",borderRadius:14,padding:"14px 0",color:C.muted,fontSize:14,fontWeight:700,cursor:"pointer"}}>Back</button>
            {previewTxs.length>0&&(
      ...
      {status==="importing"&&(
        <div style={{textAlign:"center",padding:"48px 0"}}>
          <div style={{fontSize:40,marginBottom:12}}>⏳</div>
          <div style={{fontSize:14,fontWeight:700,color:C.navy,marginBottom:4}}>Saving to Firebase...</div>
      ...
      {status==="done"&&(
        <>
          <div style={{...card,padding:"24px 20px",marginBottom:16,textAlign:"center"}}>
            <div style={{fontSize:44,marginBottom:12}}>✅</div>
            <div style={{fontSize:16,fontWeight:900,color:C.navy,marginBottom:4}}>Import Successful!</div>
            <div style={{fontSize:12,color:C.muted,marginBottom:12}}>{importedIds.length} transactions saved to your account</div>
            <div style={{fontSize:11,color:C.muted,background:C.inpBg,borderRadius:10,padding:"8px 12px"}}>Data synced to Firebase ✓</div>
          </div>
          <button onClick={handleUndo} style={{width:"100%",background:`${C.coral}12`,border:`1.5px solid ${C.coral}40`,borderRadius:16,padding:"14px 0",color:C.coral,fontSize:14,fontWeight:700,cursor:"pointer",marginBottom:10}}>
            ↩ Undo Import
          </button>
          <button onClick={reset} style={{width:"100%",background:C.border,border:"none",borderRadius:14,padding:"12px 0",color:C.muted,fontSize:13,fontWeight:600,cursor:"pointer"}}>
            Import Another File
          </button>
        </>
      )}
      ...
      {status==="undone"&&(
        <div style={{...card,padding:"24px 20px",textAlign:"center"}}>
          ...
          <div style={{fontSize:12,color:C.muted,marginBottom:16}}>All imported transactions removed</div>
          <button onClick={reset} style={{background:`${C.mint}15`,border:`1px solid ${C.mint}40`,borderRadius:12,padding:"10px 24px",color:C.mintD,fontSize:13,fontWeight:700,cursor:"pointer"}}>Import Again</button>
        </div>
      )}
    </div>
  );
